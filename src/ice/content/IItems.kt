...
object IItems : Load {
  val 脊髓末梢 = IceItem("item_spinalCordEnding", "bf3e47") {
    ...
    bundle {
      desc(zh_CN, "脊髓末梢", "一种从生物质中提取的神经传导组织,特殊处理后可用于强化机械单位的反应速度与攻击精准度")
    }
  }
  val 无名肉块 = IceItem("item_namelessCut", "bf3e47") {
    nutrientConcentration = 0.5f
    bundle {
      desc(zh_CN, "无名肉块", "一种基础血肉原料,表面布满跳动的血管", "我们以为北极只是冷,但是没想到,寒冷竟也能如此饥饿...")
    }
  }
  val 碎骨 = IceItem("item_bonesNap", "bf3e47") {
    nutrientConcentration = 0.1f
    bundle {
      desc(zh_CN, "碎骨", "一种经过粉碎处理的生物骨质残骸,表面布满裂痕与强化接合点.可作为廉价的结构填充材料,或进一步强化为高硬度复合材料")
    }
  }
  val 肌腱 = IceItem("item_muscleTendon", "bf3e47") {
    nutrientConcentration = 0.25f
    bundle {
      desc(zh_CN, "肌腱", "一种高强度的生物纤维束,经过改造后具备惊人的弹性与韧性", "牛皮糖?")
    }
  }
  ...
  val 红冰 = IceItem("item_redIce", "ff7171") {
    radioactivity = 0.05f
    bundle {
      desc(zh_CN, "红冰", "一种红色的放射性冰晶,内部含有不稳定的能量结构,在受到外部刺激时可能引发剧烈释放")
    }
  }
  ...
  val 灼热孢团 = IceItem("item_pyroSpore", "eac73e") {
    ...
    flammability = 0.4f
    bundle {
      desc(zh_CN, "灼热孢团", "一种散发着高温的孢子团,内部含有可燃物质,可作为燃料或爆炸物的原料使用")
    }
  }
  val 寂温疮体 = IceItem("item_lonelyHeatSoreSpore", "b3f1ff") {
    flammability = 0.1f
    bundle {
      desc(zh_CN, "寂温疮体", "一种奇特的低温孢子组织,表面覆盖有冰晶,可稳定吸收并传导热能,可用于冷却系统构建或低温材料的制备")
    }
  }
  val 腐败孢团 = IceItem("item_rottenSpore", "a09bbd") {
    ...
    flammability = 0.3f
    bundle {
      desc(zh_CN, "腐败孢团", "一种腐烂的孢子团块,持续散发着恶臭,可用于制造肥料或燃料")
    }
  }
  ...
  val 铬铁矿 = OreItem("item_chrome", "768a9a", 3) {
    bundle {
      desc(zh_CN, "铬铁矿", "一种重要的铬矿石,表面呈现金属至亚金属光泽.含有高比例铬成分,是生产不锈钢与耐热合金的关键工业原料")
    }
  }
  ...
  val 金矿 = OreItem("item_goldOre", "f8df87", 3) {
    bundle {
      desc(zh_CN, "金矿", "一种含有金元素的珍贵矿石,虽整体含量虽不高,但仍是提取贵重金属的核心来源")
    }
  }
  val 锆英石 = OreItem("item_azorite", "8c3e2d", 4) {
    bundle {
      desc(zh_CN, "锆英石", "一种坚硬的硅酸盐矿物,呈短柱状,广泛用于陶瓷工业与耐火材料制备.是提取锆的主要原料之一")
    }
  }
  val 硫钴矿 = OreItem("item_linnaeite", "cfecf1", 3) {
    bundle {
      desc(zh_CN, "硫钴矿", "一种银白色金属光泽,常带粉红色调,立方体或八面体晶形.是提炼钴的主要原料之一", "这玩意儿比钻石难啃\n——采矿队长R-42的日志")
    }
  }
  val 铱锇矿 = OreItem("item_iridiumosm", "656565", 4) {
    bundle {
      desc(zh_CN, "铱锇矿", "一种稀有金属矿,表面呈黯淡金属光泽,含有铱和锇,是提炼这两种金属的主要原料之一")
    }
  }
  val 铈硅石 = OreItem("item_cerite", "BFC8E2", 2) {
    bundle {
      desc(zh_CN, "铈硅石", "一种稀土矿物,常呈黄褐色至红褐色.含有较高比例的铈元素,是提炼铈的主要原料之一")
    }
  }
  val 铀原矿 = OreItem("item_uranium_rawore", "#95B564", 4) {
    ...
    radioactivity = 0.04f
    bundle {
      desc(zh_CN, "铀原矿", "一种常见的半晶质铀矿物,呈浅绿色沥青光泽,经处理离心后可分离出铀的两种常见同位素", "尽管是放射性元素的矿物,但在矿石中的铀密度还不足以产生致命的辐射...什么？你问我为什么手上的小盒子在哒哒响？")
    }
  }
  ...
  val 低碳钢 = IceItem("item_lowCarbonSteel", "d4d7e4") {
    bundle {
      desc(zh_CN, "低碳钢", "一种含碳量较低的钢材,具有优异的塑性和韧性,是建筑和机械制造的常用基础材料")
    }
  }
  val 高碳钢 = IceItem("item_highCarbonSteel", "bedfee") {
    bundle {
      desc(zh_CN, "高碳钢", "一种含碳量较高的钢材,硬度优于低碳钢,适合用于制造需要高强度的建筑和部件")
    }
  }
  ...
  val 铅锭 = IceItem("item_leadIngot", "8c7fa9") {
    bundle {
      desc(zh_CN, "铅锭", "一种高密度金属,具有出色的辐射防护能力,既可用于防护设施,也可作为重型武器的材料", "从挡住辐射到砸穿装甲,这份量从来不会让人失望")
    }
  }
  ...
  val 铪锭 = IceItem("item_hafniIngot", "f7e5f3") {
    bundle {
      desc(zh_CN, "铪锭", "一种稀有的高熔点金属,在能量传导和高温环境中表现出色,是高级能量设备的关键材料")
    }
  }
  val 铈锭 = IceItem("item_ceriumIngot", "BFC8E2") {
    ...
    healthScaling = 0.6f
    bundle {
      desc(zh_CN, "铈锭", "一种经过初步加工的铈矿石,用于研磨抛光剂,特种玻璃及推进器零件等")
    }
  }
  ...
  val 钴钢 = IceItem("item_cobaltSteel", "c5d1e0") {
    bundle {
      desc(zh_CN, "钴钢", "一种钴合金钢材,具备优异的耐高温与耐腐蚀性能,质地坚硬且结构稳定.常用于高强度建筑结构及防护装备的制造")
    }
  }
  ...
  val 爆炸化合物 = IceItem("item_explosiveCompound", "ff795e") {
    ...
    buildable = false
    bundle {
      desc(zh_CN, "爆炸化合物", "一种极端不稳定的红色粉末,为满足冲击武器需求而诞生的高能材料,爆炸威力极强,应避免长途运输与存储")
    }
  }
  ...
  val 铈凝块 = IceItem("item_ceriumClot", "929DB5") {
    ...
    flammability = 3.6f
    bundle {
      desc(zh_CN, "铈凝块", "一种对碰撞和温度极为敏感的金属凝块,暴露于空气中极易自燃,需严格密封保存")
    }
  }
  val 铱板 = IceItem("item_iridiumPlate", "656565") {
    ...
    healthScaling = 1f
    bundle {
      desc(zh_CN, "铱板", "一种已经完成了压铸工序的铱合金板材,具备极强的抗冲击性能,是一种优秀的各向异性介质,适用于精密核电路及高性能防护结构的制造")
    }
  }
  val 燃能晶 = IceItem("item_burningCrystal", "737373") {
    bundle {
      desc(zh_CN, "燃能晶", "一种经过特殊工艺处理的晶体燃料,燃烧时可释放巨大能量,是驱动各类工业设备的核心能源材料")
    }
  }
  val 石英玻璃 = IceItem("item_quartzGlass", "ebeef5") {
    bundle {
      desc(zh_CN, "石英玻璃", "一种由高纯度石英熔制而成的玻璃材料,具备优异的耐高温性能和极低的热膨胀系数,能够有效抵御大部分化学反应侵蚀,是理想的反应容器")
    }
  }
  val 复合陶瓷 = IceItem("item_compositeCeramic", "ebeef5") {
    bundle {
      desc(zh_CN, "复合陶瓷", "一种凭借超凡抗冲击性与热稳定性著称的工程材料.既可作为能量护盾发生器的核心基底材料,又能制成高速单位所需的耐热防护瓦")
    }
  }
  val 陶钢 = IceItem("item_potterySteel", "D6DEC6") {
    ...
    healthScaling = 0.8f
    bundle {
      desc(zh_CN, "陶钢", "一种复合装甲材料,能够快速且均匀地将电磁粒子辐射分散传导至装甲各处,从而大幅降低高强度动能冲击造成的局部破坏")
    }
  }
  val 石墨烯 = IceItem("item_graphene", "52578a") {
    bundle {
      desc(zh_CN, "石墨烯", "一种超薄的单层碳原子材料,具有出色的导电性和机械强度,是高级电子设备的核心材料")
    }
  }
  ...
  val 导能回路 = IceItem("item_conductingCircuit", "867F8C") {
    bundle {
      desc(zh_CN, "导能回路", "一种将高纯度单晶硅回路蚀刻于放射性能级降低的钍基座中制成的能量传导组件,极大提升其导能性与稳固性")
    }
    charge = 0.8f
    cost = 1.5f
    healthScaling = 0.5f
  ...
  val 电子元件 = IceItem("item_integratedCircuit", "53565c") {
    bundle {
      desc(zh_CN, "电子元件", "一种精密的电子器件,是制造高级设备和处理器的基础材料")
    }
  }
  val 肃正协议 = IceItem("item_solemnProtocol", "FF5845") {
    cost = 600f
    bundle {
      desc(zh_CN, "肃正协议", "一种装载特定密匙的激活协议,授权后即可解锁高级军械的控制权限与制造权限")
    }
  }
  val 生物钢 = IceItem("item_biologicalSteel", "D75B6E") {
    bundle {
      desc(zh_CN, "生物钢", "一种被强行抑制了绝大部分活性的生物组织材料.此材料制造的装甲具有整体无缝,附着性强,耐酸碱,防辐射,防腐蚀,耐冲击等的优异特性,只是....", "你真的要使用他吗?")
    }
    frames = 3
    transitionFrames = 13
    frameTime = 5f
  ...
  val 暮光合金 = IceItem("item_duskIngot", "deedff") {
    bundle {
      desc(zh_CN, "暮光合金", "一种在暮光时分呈现特殊光泽的合金材料,具有优异的能量传导特性,是高级能量设备的核心材料")
    }
  }
  ...
  val 玳渊矩阵 = IceItem("item_abyssMatrix", "d7bdff") {
    bundle {
      desc(zh_CN, "玳渊矩阵", "一种具有独特的能量传导和存储特性,是高级炮塔弹药的核心材料")
    }
  }
  ...
  val 矩阵合金 = IceItem("item_matrix_alloy", "#929090") {
    ...
    cost = 1.4f
    bundle {
      desc(zh_CN, "矩阵合金", "一种由大量纳米机器人聚合而成的可编程材料,机械强度较低,但能够自由变换形态甚至物态,具备极高的适应性与灵活性")
    }
  }
  val 强化合金 = IceItem("item_strengthening_alloy", "#B1B1B0") {
    ...
    cost = 1.25f
    bundle {
      desc(zh_CN, "强化合金", "一种各项性能均衡优异的合金材料,具备极高的结构强度与抗性,广泛应用于各类领域")
    }
  }
  val 气凝胶 = IceItem("item_aerogel", "#D5EBEE") {
    ...
    cost = 1.1f
    bundle {
      desc(zh_CN, "气凝胶", "一种硅基结构的超轻材料.内部由大量细微蜂窝状孔洞填充,具备强度高,密度极小,绝缘绝热等优异特性,广泛应用于各领域")
    }
  }
  val 简并态中子聚合物 = IceItem("item_degenerate_neutron_polymer", "#FF7FE0") {
    ...
    cost = 3f
    bundle {
      desc(zh_CN, "简并态中子聚合物", "一种由中子简并态构成的超高密度材料,硬度难以测量,韧性与塑性趋近于零.除作为极端环境下的结构材料外,亦是引力场技术的理想场源")
    }
  }
  val 铀238 = IceItem("item_uranium_238", "#7CA73D") {
    ...
    cost = 1.5f
    bundle {
      desc(zh_CN, "铀-238", "一种俗称贫铀的放射性同位素,硬度较高且结构致密.可用作中子反射板及动能穿甲弹头,亦可吸收中子后衰变为更有用的钚-239", "无论在哪一个星球上,自然状态下的铀238总是占据铀的绝大部分的丰富度,所幸它可以转变为更有用的东西,如果全部都用来造穿甲弹和防弹装甲之类的话,保有量可能可以用到下个地质纪...")
    }
  }
  ...
  val 钚239 = IceItem("item_plutonium_239", "#D1D19F") {
    ...
    radioactivity = 1.6f
    bundle {
      desc(zh_CN, "钚-239", "一种由铀-238吸收中子后衰变而成的强放射性同位素,与铀-235同为常用的裂变核燃料", "尽管铀238可以转换为钚239,但也许你需要一个不小规模的反应堆阵列才能实现量产钚239了")
    }
  }
  ...
  val 浓缩铀235核燃料 = IceItem("item_concentration_uranium_235", "#95B564") {
    ...
    radioactivity = 2.4f
    bundle {
      desc(zh_CN, "封装铀-235", "一种经高度浓缩并超低温封装的铀-235核燃料,可在超过常温的临界压缩状态下稳定存储,是反应堆的主要燃料之一")
    }
  }
  val 浓缩钚239核燃料 = IceItem("item_concentration_plutonium_239", "#B0B074") {
    ...
    radioactivity = 2.4f
    bundle {
      desc(zh_CN, "封装钚-239", "一种经高度浓缩并超低温封装的钚-239核燃料,可在超过常温的临界压缩状态下稳定存储,是反应堆的主要燃料之一")
    }
  }
  val 氢聚变燃料 = IceItem("item_hydrogen_fusion_fuel", "#83D6A0") {
    ...
    radioactivity = 0f
    bundle {
      desc(zh_CN, "氢聚变燃料", "一种压缩氢原子同位素,原子性质易于发生聚变,属轻核聚变燃料")
    }
  }
  val 氦聚变燃料 = IceItem("item_helium_fusion_fuel", "#D0D6B7") {
    ...
    radioactivity = 0f
    bundle {
      desc(zh_CN, "氦聚变燃料", "一种压缩氦原子同位素,原子性质易于发生聚变,属轻核聚变燃料")
    }
  }
  val 反物质 = IceItem("item_anti_metter", "734CD2") {
    ...
    radioactivity = 0f
    bundle {
      desc(zh_CN, "反物质", "一份由引力场约束隔绝的反物质,与正物质接触即发生完全湮灭并将质量转化为纯能量,危险性极高,广泛用于强攻击性武器", "通常每一个反物质储存单元都会具有一个独立的能源模块来维持约束力场,在远航星舰队伴随燃料仓库放出的耀眼的焰火消失后,停电被成为了携带反物质的情况下最危险的事")
    }
  }
  ...
  val 碱石 = IceItem("item_alkali_stone", "#B0BAC0") {
    ...
    radioactivity = 0f
    bundle {
      desc(zh_CN, "碱石", "一种以碱石灰为主要成分的矿物,内部富含多种含氯金属盐类.用于电离处理分离出碱液与氯气", "通常来说,河流会携带附着在河床上的盐类物质,并汇入海洋或者湖泊,长久的积累使得水体中的盐浓度不断升高,进而形成高含盐量的海洋和咸水湖,绝大多数表面具有活跃水圈的星球都是符合这样的规律")
    }
  }
  val 絮凝剂 = IceItem("item_flocculant", "ffffff") {
    ...
    radioactivity = 0f
    bundle {
      desc(zh_CN, "絮凝剂", "一种投入水中可形成大量多孔絮状胶质的化工材料,用于吸附或者分离固形物质,是重要的化工材料", "注意防潮\n请于阴凉环境储存,避免阳光直射\n保质期：六个月\n若不慎误食,请立即就医并向医师提供此说明书")
    }
  }
  ...
  val 铱金混合物 = IceItem("item_iridium_mixed_rawmaterial", "#AECBCB") {
    ...
    radioactivity = 0f
    bundle {
      desc(zh_CN, "铱金混合物", "一种含有铱金属氧化物固体混合物,经过加工提纯可以获得冶炼铱的进一步中间物", "在足够高压的核裂变反应中,原子核的衰变和四处弹射的中子总是能带来一些令人意想不到的东西,它们会残留在核废料里,等待着变成更有用的东西")
    }
  }
