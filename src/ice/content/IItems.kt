...
object IItems :Load {
  ...
  val 金珀沙 = IceItem("item_goldSand", "f8efad") {
    ...
    localization {
      zh_CN {
        name = "金珀沙"
        description = "一种闪耀着金色光泽的沙粒,其中混有微量琥珀结晶,在光照下会浮现半透明的琥珀色光晕.经适当工艺处理后,可从中提取出多种矿物质"
      }
    }
  }
  ...
  val 焦炭 = IceItem("item_coke", "#6A6A69") {
    ...
    localization {
      zh_CN {
        name = "焦炭"
        description = "一种经处理后的碳质材料,相较于原煤具有更高的碳含量与热值,杂质少,燃烧更为彻底,在冶炼工业中作为补充热源及碳素材料被广泛使用"
      }
    }
  }
  ...
  val 硫化合物 = IceItem("item_sulfurCompound", "ffaa5f") {
    ...
    localization {
      zh_CN {
        name = "硫化合物"
        description = "硫素与金属化合而成的晶体常态下相对平和,但其受热分解后伴随发生着地急剧化学反应,足以融毁多数\n能够迸发出大量热能的可控燃素显然重要,因而硫化物得以被视作为一种重要材料,为工业或武器的方方面面所利用"
      }
    }
  }
  ...
  val 碱石 = IceItem("item_alkali_stone", "#B0BAC0") {
    localization {
      zh_CN {
        ...
        details = "通常来说,河流会携带附着在河床上的盐类物质,并汇入海洋或者湖泊,长久的积累使得水体中的盐浓度不断升高,进而形成高含盐量的海洋和咸水湖,绝大多数表面具有活跃水圈的星球都是符合这样的规律"
      }
    }
  }
  ...
  val 岩层沥青 = IceItem("item_rock_bitumen", "#808A73") {
    localization {
      zh_CN {
        ...
        details = "矿石碎屑在某些条件下会自发的富集并成块,大部分都会形成矿层,但许多未成型的矿屑被粘度很高的胶体裹挟时会在深层底层中形成沥青状的矿石胶结物,成分复杂"
      }
    }
  }
  ...
  val 铱金混合物 = IceItem("item_iridium_mixed_rawmaterial", "#AECBCB") {
    localization {
      zh_CN {
        ...
        details = "在足够高压的核裂变反应中,原子核的衰变和四处弹射的中子总是能带来一些令人意想不到的东西,它们会残留在核废料里,等待着变成更有用的东西"
      }
    }
  }
  ...
  val 铀238 = IceItem("item_uranium_238", "#7CA73D") {
    ...
    localization {
      zh_CN {
        ...
        details = "无论在哪一个星球上,自然状态下的铀238总是占据铀的绝大部分的丰富度,所幸它可以转变为更有用的东西,如果全部都用来造穿甲弹和防弹装甲之类的话,保有量可能可以用到下个地质纪..."
      }
    }
  }
  ...
  val 相位封装氢单元 = IceItem("item_encapsulated_hydrogen_cell", "#9EFFC6") {
    ...
    localization {
      zh_CN {
        ...
        details = "相位物的中子光路学结构会将中子聚焦到中心存储氢的空腔,以最大化中央接收到的中子流,在中子流的轰击下,大量氢原子会转化为较为容易发生核聚变反应的同位素,继而参与核聚变"
      }
    }
  }
  val 相位封装氦单元 = IceItem("item_encapsulated_helium_cell", "#F9FFDE") {
    ...
    localization {
      zh_CN {
        ...
        details = "相位物的中子光路学结构会将中子聚焦到中心存储氦的空腔,以最大化中央接收到的中子流,在中子流的轰击下,大量氦原子会转化为较为容易发生核聚变反应的同位素,继而参与核聚变"
      }
    }
  }
  ...
  val 生物钢 = IceItem("item_biologicalSteel", "D75B6E") {
    localization {
      zh_CN {
        name = "生物钢"
        description = "一种被强行抑制了绝大部分活性的生物材料.此材料制造的装甲具有整体无缝,附着性强,耐酸碱,防辐射,防腐蚀,耐冲击等的优异特性,只是...."
        details = "你真的要使用他吗?"
      }
    }
    frames = 3
  ...
  val 简并态中子聚合物 = IceItem("item_degenerate_neutron_polymer", "#FF7FE0") {
    localization {
      zh_CN {
        name = "简并态中子聚合物"
        description = "由中子简并态构成的超高密度聚合物,硬度难以测量,韧性与塑性趋近于零.除作为极端环境下的结构材料外,亦是引力场技术的理想场源"
      }
    }
  }
  ...
  val 以太能 = IceItem("item_etherealEnergy", "E6C4EE") {
    localization {
      zh_CN {
        name = "以太能"
        description = "封装容器中的高能粒子能量,在特定结构排列下注入相位能量后可影响时空结构.研究初期曾引发时空回溯,空间错位及乱序传送等现象"
        details = "在以太粒子以特定结构排列时注入相位能量,以太粒子会在法韦克内敛空间的能量辐射下,形成以伊塔宏粒子射线为场能的波态中子向心力场"
      }
    }
    frames = 2
    transitionFrames = 24
  ...
  val 反物质 = IceItem("item_anti_metter", "734CD2") {
    ...
    localization {
      zh_CN {
        ...
        details = "通常每一个反物质储存单元都会具有一个独立的能源模块来维持约束力场,在远航星舰队伴随燃料仓库放出的耀眼的焰火消失后,停电被成为了携带反物质的情况下最危险的事"
      }
    }
  }
}
