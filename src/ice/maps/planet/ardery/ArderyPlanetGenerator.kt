...
class ArderyPlanetGenerator : PlanetGenerator() {
  ...
  fun rawHeight(position: Vec3): Float {
    return (Mathf.pow(
      Simplex.noise3d(
        seed, 7.0, 0.5, (1f / 3f).toDouble(), (position.x * scl).toDouble(),
        (position.y * scl + heightYOffset).toDouble(), (position.z * scl).toDouble()
      ) * heightScl,
      2.3f
  ...
  override fun getEmissiveColor(position: Vec3, out: Color) {
    ...
    IPlanets.阿德里.sectors.forEach { sector ->
      if (sector.hasEnemyBase() && !sector.isCaptured) {
        dst = min(
          dst, position.dst(
            sector.tile.v
          ) - (if (sector.preset != null) sector.preset.difficulty / 10f * 0.03f - 0.03f else 0f)
        )
    ...
    val freq = 0.05f
    if (if (position.dst(basePos) < 0.55f) dst * metalDstScl + Simplex.noise3d(
        seed + 1, 3.0, 0.4, 5.5,
        position.x.toDouble(), (position.y + 200f).toDouble(),
        position.z.toDouble()
      ) * 0.08f + (if ((basePos.dst(
          position
        ) + 0.00f) % freq < freq / 2f
      ) 1f else 0f) * 0.07f < 0.08f /* || dst <= 0.0001f*/ else dst * metalDstScl + Simplex.noise3d(
        seed, 3.0,
        0.4, 9.0, position.x.toDouble(), (position.y + 370f).toDouble(), position.z.toDouble()
      ) * 0.06f < 0.045
    ) {
      out.set(Team.crux.color)
        .mul(
          0.8f + Simplex.noise3d(
            seed, 1.0, 1.0, 9.0, position.x.toDouble(), (position.y + 99f).toDouble(),
            position.z.toDouble()
          ) * 0.4f
        )
        .lerp(
          Team.sharded.color,
          0.2f * Simplex.noise3d(
            seed, 1.0, 1.0, 9.0, position.x.toDouble(), (position.y + 999f).toDouble(),
            position.z.toDouble()
          )
        ).toFloatBits()
    } else if (captureDst * metalDstScl + Simplex.noise3d(
        seed, 3.0, 0.4, 9.0, position.x.toDouble(),
        (position.y + 600f).toDouble(), position.z.toDouble()
      ) * 0.07f < 0.05 * lightScl
    ) {
      out.set(Team.sharded.color).mul(
        0.7f + Simplex.noise3d(
          seed, 1.0, 1.0, 9.0, position.x.toDouble(), (position.y + 99f).toDouble(),
          position.z.toDouble()
        ) * 0.4f
      )
  ...
  public override fun genTile(position: Vec3, tile: TileGen) {
    ...
    if (Ridged.noise3d(
        seed + 1, position.x.toDouble(), position.y.toDouble(), position.z.toDouble(), 2,
        22f
      ) > 0.31
    ) {
  ...
  fun getBlock(position: Vec3): Block {
    ...
    val tnoise = Simplex.noise3d(
      seed, 7.0, 0.56, (1f / 3f).toDouble(), px.toDouble(),
    ...
    val tar = Simplex.noise3d(
      seed, 4.0, 0.55, (1f / 2f).toDouble(), px.toDouble(), (py + 999f).toDouble(),
      pz.toDouble()
    ) * 0.3f + position.dst(0f, 0f, 1f) * 0.2f
    ...
    if (tar > 0.5f) {
      ...
      if (position.within(basePos, 0.65f)) {
        ...
        if (dst * 0.85f + Simplex.noise3d(
            seed, 3.0, 0.4, 5.5, position.x.toDouble(),
            (position.y + 200f).toDouble(), position.z.toDouble()
          ) * 0.015f + (if ((basePos.dst(
              position
            ) + 0.00f) % freq < freq / 2f
  ...
  override fun noise(x: Float, y: Float, octaves: Double, falloff: Double, scl: Double, mag: Double): Float {
    ...
    return Simplex.noise3d(
      seed, octaves, falloff, 1f / scl, v.x.toDouble(), v.y.toDouble(),
      v.z.toDouble()
    ) * mag.toFloat()
  }

 fun interface Tmps : TileHeuristic {
   override fun cost(from: Tile?, tile: Tile?): Float {
     return cost(tile)
   }
  }

   fun pathfind(startX: Int, startY: Int, endX: Int, endY: Int, th: Tmps, dh: DistanceHeuristic): Seq<Tile?> {
    return Astar.pathfind(startX, startY, endX, endY, th, dh) { tile: Tile? -> Vars.world.getDarkness(tile!!.x.toInt(), tile.y.toInt()) <= 1f }
  }
  ...
  override fun generate() {
    class Room(var x: Int, var y: Int, var radius: Int) {
      ...
      fun join(x1: Int, y1: Int, x2: Int, y2: Int) {
        ...
        var a=Tmps{ tile: Tile? ->
          (if (tile!!.solid()) 50f else 0f) + noise(
            tile.x.toFloat(), tile.y.toFloat(), 2.0, 0.4, (1f / nscl).toDouble()
          ) * 500
        }
        brush(pathfind(x1, y1, x2, y2,a , Astar.manhattan), stroke)
      }
        ...
        path.each(Cons { t ->
          ...
          for (x in -rad..rad) {
            for (y in -rad..rad) {
              val wx = t.x + x
              val wy = t.y + y
              if (Structs.inBounds(wx, wy, width, height) && Mathf.within(
                  x.toFloat(), y.toFloat(),
                  rad.toFloat()
                )
              ) {
    ...
    pass { x: Int, y: Int ->
      ...
      val rr = Simplex.noise2d(
        sector.id, 2f.toDouble(), 0.6, (1f / 7f).toDouble(), x.toDouble(),
        y.toDouble()
      ) * 0.1f
      ...
      if (value > 0.17f && !Mathf.within(
          x.toFloat(), y.toFloat(), fspawn!!.x.toFloat(), fspawn.y.toFloat(),
          12 + rrscl
        )
      ) {
        val deep = value > 0.17f + 0.1f && !Mathf.within(
          x.toFloat(), y.toFloat(), fspawn.x.toFloat(),
          fspawn.y.toFloat(), 15 + rrscl
        )
        val sd = value < 0.17f + 0.05f && !Mathf.within(
          x.toFloat(), y.toFloat(), fspawn.x.toFloat(),
          fspawn.y.toFloat(), 15 + rrscl
        )
        //不要把河流放在冰上，它们是结冰的
    ...
    /*  //海岸线设置
      pass { x: Int, y: Int ->
          ...
          if (floor.asFloor().isLiquid && floor.asFloor().shallow) {
              ...
              floor = if (floor === _root_ide_package_.ice.content.block.Environment.血浅滩) _root_ide_package_.ice.content.block.Environment.血池 else Blocks.water
          }
      }*/
    /* if (naval) {
         val deepRadius = 2
    ...
    Vars.state.rules.spawns = Waves.generate(
      difficulty, Rand(sector.id.toLong()), Vars.state.rules.attackMode,
      Vars.state.rules.attackMode && Vars.spawner.countGroundSpawns() == 0, false
    )//naval)
  }
  ...
  fun setOres() {
    ...
    pass { x: Int, y: Int ->
      ...
      for (i in ores.size - 1 downTo 0) {
        ...
        if (abs(
            0.5f - noise(
              offsetX.toFloat(), (offsetY + i * 999).toFloat(), 2.0, 0.7,
              (40 + i * 2).toDouble()
            )
          ) > 0.22f + i * 0.01 &&
          abs(
            0.5f - noise(
              offsetX.toFloat(), (offsetY - i * 999).toFloat(), 1.0, 1.0,
              (30 + i * 4).toDouble()
            )
          ) > 0.37f + freq
        ) {
  ...
  override fun postGenerate(tiles: Tiles?) {
    if (sector.hasEnemyBase()) {
      ...
      if (Vars.spawner.countGroundSpawns() == 0) {
        Vars.state.rules.spawns = Waves.generate(
          sector.threat, Rand(sector.id.toLong()),
