...
class CarryTaskAI :AIController() {
  ...
  data class ItemTask(val source: DroneDeliveryTerminalBuild, val target: Building, val items: ItemStack)
  ...
  override fun updateMovement() {
    ...
    task?.let { currentTask ->
      ...
      if (hasCorrectItems(unit.stack, currentTask.items)) {
        deliverToTarget(currentTask)
      } else {
        // 否则前往源建筑获取物品
        collectFromSource(currentTask)
      }
      return
    }
    // 没有任务时，返回源建筑附近
    ...
    if (task==null &&unit.hasItem())unit.clearItem()
  }
  ...
  private fun deliverToTarget(task: ItemTask) {
    ...
    if (task.target.within(unit, TRANSFER_DISTANCE)) {
      Call.transferItemTo(
        unit, task.items.item, task.items.amount, task.target.x, task.target.y, task.target
      )
      clearTask()
    }
  }
  ...
  private fun collectFromSource(task: ItemTask) {
    source?.let { sourceBuilding ->
      // 检查源建筑是否有足够的物品
      if (sourceBuilding.items.has(task.items.item, task.items.amount)&&!unit.hasItem()) {
