...
object ModInfoDialog : BaseMenusDialog(IceStats.模组.localized(), IStyles.menusButton_infos) {
    ...
    override fun build(cont: Table) {
        ...
        cont.iPaneG { ta ->
            ta.table {
                it.layoutLabel("主要贡献者:").fontScale(1.4f).row()
            ...
            ta.iTable {
                var deepSpaceURI="https://github.com/Icexuegao/DeepSpace"
                it.layoutLabel(deepSpaceURI).tapped {
                    Core.app.openURI(deepSpaceURI)
                }
            }.row()
