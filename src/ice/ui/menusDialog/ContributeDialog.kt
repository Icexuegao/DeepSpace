package ice.ui.menusDialog

import arc.Core
import arc.graphics.Color
import arc.scene.style.Drawable
import arc.scene.ui.Button
import arc.scene.ui.layout.Table
import arc.util.Scaling
import ice.graphics.IStyles
import ice.ui.dialog.BaseMenusDialog
import ice.world.meta.IceStats
import mindustry.gen.Icon
import mindustry.gen.Tex
import mindustry.graphics.Pal
import singularity.Sgl
import singularity.graphic.SglDrawConst
import singularity.ui.SglStyles

object ContributeDialog : BaseMenusDialog(IceStats.捐赠.localized(), IStyles.menusButton_contribute) {
  override fun build(cont: Table) {
    cont.table(SglDrawConst.grayUIAlpha) { main: Table? ->
      main!!.pane { p: Table? ->
        p!!.defaults().growX().pad(5f)
        p.add(Core.bundle.get("dialog.support.info")).growX().wrap()
        p.row()
        p.image().color(Pal.accent).height(4f).pad(0f).padTop(4f).padBottom(8f).growX()
        p.row()
        p.image(SglDrawConst.sgl2).scaling(Scaling.fit).size(365f)
        p.row()
        p.add(Core.bundle.get("dialog.support.star")).growX().wrap()
        p.row()
        buildButton(p, Icon.github, Pal.accent, "GitHub", Core.bundle.get("dialog.support.githubStar")) { Core.app.openURI(Sgl.githubProject) }
        p.row()
        p.add(Core.bundle.get("dialog.support.donate")).padTop(20f).growX().wrap()
        p.row()
        buildButton(p, Icon.none, Pal.reactorPurple, Core.bundle.get("misc.afdian"), Core.bundle.get("dialog.support.afdian"), Runnable {})
        p.row()
        buildButton(p, Icon.none, Pal.lancerLaser, "Patreon", Core.bundle.get("dialog.support.patreon")) {}
      }.growX().fillY()
    }.margin(12f).growX().fillY().maxWidth(735f)
  }
  private fun buildButton(table: Table, icon: Drawable?, color: Color, name: String?, subText: String?, listener: Runnable?) {
    table.button({ b: Button? ->
      b!!.left().defaults().left().padBottom(-12f)
      b.table { img: Table? ->
        img!!.image().growY().width(30f).color(color)
        img.row()
        img.image().height(6f).width(30f).color(color.cpy().mul(0.8f, 0.8f, 0.8f, 1f))
      }.growY().fillX().padLeft(-12f)

      b.table(Tex.buttonEdge3) { i: Table? -> i!!.image(icon).size(55f) }.size(64f)
      b.table { t: Table? ->
        t!!.defaults().left().growX()
        t.add(name).color(Pal.accent)
        t.row()
        t.add(subText).color(Pal.gray)
      }.grow().padLeft(5f)
    }, SglStyles.underline, listener)
  }
}