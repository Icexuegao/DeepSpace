...
class UCLinkNode(
  ...
  fun setupRequirements() {
    ...
    for (i in requirements.indices) {
      finishedRequirements?.set(
        i, ItemStack(
          requirements[i].item, if (Core.settings == null) 0 else Core.settings.getInt(
          ...
          items.add(object : Table() {
            override fun drawChildren() {
              ...
              if (size > 1) {
                ...
                Lines.line(
                  x + marginLeft, y + height / 2f, x + (width - 2 * marginLeft), y + height / 2f
                )
              }
              super.drawChildren()
            }
              ...
              it.iTableG { it ->
                val flun = {
                  ...
                  it.add(Label("")).update {
                    ...
                    val color = if (rt == 0) Pal.remove else if (rt < itemsStacks[i].amount) IceColor.b3 else IceColor.b4
                    it.setColor(color)
