...
class DeathGiftAbility(var range: Float, var status: StatusEffect, var duration: Float, var percent: Float, var amount: Float) :Ability() {
  ...
  override fun addStats(table: Table) {
    ...
    table.add("[#${Pal.accent}]${range.toTrimmedString(1)}[] ${IceStats.范围.localized()}").row()
    table.add("[#${Pal.accent}]${status.localizedName}[] ${IceStats.状态.localized()}").row()
    table.add("[#${Pal.accent}]${(duration / 60f).toTrimmedString(1)}[] ${IceStats.秒.localized()}").row()
    table.add("[#${Pal.accent}]${(percent * 100).toTrimmedString(1)}%[] ${IceStats.修复.localized()}").row()
    table.add("[#${Pal.accent}]${(amount).toTrimmedString(1)}[] ${IceStats.修复量.localized()}").row()
  }
