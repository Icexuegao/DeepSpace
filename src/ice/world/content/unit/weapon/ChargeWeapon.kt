...
class ChargeWeapon(name: String = "") : Weapon(name) {
  ...
  var reloadMultiplierSpeedDecrease = 0.01f

  override fun update(unit: Unit, mount: WeaponMount) {
    val can = unit.canShoot()
    if (mount.shoot) {
      mount.reloadMultiplier = Mathf.approachDelta(mount.reloadMultiplier, 1f, reloadMultiplierSpeed)
    } else {
      mount.reloadMultiplier = Mathf.approachDelta(mount.reloadMultiplier, 0f, reloadMultiplierSpeedDecrease)
    }

    val lastReload = mount.reload
    mount.reload = max(mount.reload - Time.delta * unit.reloadMultiplier * mount.reloadMultiplier, 0f)
