...
open class MayflyWeapon(name: String) : DataWeapon(name) {
    ...
    bullet = object : BulletType() {
      ...
      override fun updateHoming(b: Bullet) {
        ...
        if (b.time >= homingDelay) {
          ...
          val target = if (b.aimTile != null && b.aimTile.build != null && b.aimTile.build.team !== b.team && !b.hasCollided(
              b.aimTile.build.id
            )
          ) {
            b.aimTile.build
          } else {
            Units.closestTarget(b.team, realAimX, realAimY, homingRange, Boolf { e: Unit? -> e != null && !b.hasCollided(e.id) }, Boolf { t: Building? -> t != null && !b.hasCollided(t.id) })
          }
          ...
          if (target != null) {
            ...
            b.vel.setAngle(
              Angles.moveToward(
                b.rotation(), b.angleTo(target), homingPower * (v / speed) * Time.delta * 50f
              )
            )
          } else {
            b.vel.lerpDelta(0f, 0f, 0.06f)
  ...
  override fun update(unit: Unit, mount: DataWeaponMount) {
    ...
    mount.status?.update(unit, mount)
  }

  override fun draw(unit: Unit, mount: WeaponMount) {
    if (mount is DataWeaponMount) mount.status?.draw(unit, mount)
  }

  override fun shoot(
    unit: Unit, mount: WeaponMount, shootX: Float, shootY: Float, rotation: Float
  ) {
    if (mount is DataWeaponMount) {
      Time.run(delay) {
