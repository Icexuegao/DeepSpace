...
class SolidPump(name: String) : SglBlock(name) {
  ...
  inner class SolidPumpBuild : SglBuilding() {
    ...
    override fun updateTile() {
      ...
      if (consumer.consEfficiency > 0 && typeLiquid() < liquidCapacity - 0.001f) {
        val maxPump = min(liquidCapacity - typeLiquid(), pumpAmount * delta() * fraction * consumer.consEfficiency)
