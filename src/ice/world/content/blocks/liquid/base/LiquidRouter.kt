...
open class LiquidRouter(name: String) : LiquidBlock(name) {
  ...
  init {
    ...
    drawers = DrawMulti(DrawRegionNull("-bottom"), DrawBuild<LiquidRouterBuild> {
      if (liquids.currentAmount() > 0.001f) {
        mindustry.world.blocks.liquid.LiquidBlock.drawTiledFrames(
          size, x, y, liquidPadding, liquids.current(), liquids.currentAmount() / liquidCapacity
        )
      }
