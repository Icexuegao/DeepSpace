...
open class PowerNode(name: String) : PowerBlock(name) {
  companion object {
    ...
    fun makePowerBalance(): Func<Building, Bar> {
      return Func { entity ->
        Bar({
          Core.bundle.format(
            "bar.powerbalance", ((if (entity.power.graph.powerBalance >= 0) "+" else "") + UI.formatAmount((entity.power.graph.powerBalance * 60).toLong()))
          )
        }, { Pal.powerBar }, { Mathf.clamp(entity.power.graph.lastPowerProduced / entity.power.graph.lastPowerNeeded) })
      }
    }
    ...
    fun makeBatteryBalance(): Func<Building, Bar> {
      return Func { entity ->
        Bar({
          Core.bundle.format(
            "bar.powerstored", (UI.formatAmount(entity.power.graph.lastPowerStored.toLong())), UI.formatAmount(entity.power.graph.lastCapacity.toLong())
          )
        }, { Pal.powerBar }, { Mathf.clamp(entity.power.graph.lastPowerStored / entity.power.graph.lastCapacity) })
      }
    }
    ...
    fun getNodeLinks(tile: Tile, block: Block, team: Team, others: Cons<Building>) {
      val valid = Boolf { other: Building? ->
        other != null && other.tile !== tile && other.block is PowerNode && (block as PowerNode).autolink && other.power.links.size < block.maxNodes && block.overlaps(other.x, other.y, tile, block, block.laserRange * Vars.tilesize) && other.team === team && !graphs.contains(other.power.graph) && !insulated(tile, other.tile) && !Structs.contains(Edges.getEdges(block.size)) { p: Point2? ->  //do not link to adjacent buildings
  ...
  override fun setBars() {
    ...
    addBar("connections") { entity: Building? ->
      Bar({ Core.bundle.format("bar.powerlines", entity!!.power.links.size, maxNodes) }, { Pal.items }, { entity!!.power.links.size.toFloat() / maxNodes.toFloat() })
    }
  }
  ...
  protected fun overlaps(srcx: Float, srcy: Float, other: Tile, otherBlock: Block, range: Float): Boolean {
    return Intersector.overlaps(
      Tmp.cr1.set(srcx, srcy, range), Tmp.r1.setCentered(
        other.worldx() + otherBlock.offset, other.worldy() + otherBlock.offset, (otherBlock.size * Vars.tilesize).toFloat(), (otherBlock.size * Vars.tilesize).toFloat()
      )
    )
  }
  ...
  protected fun getPotentialLinks(tile: Tile, team: Team, others: Cons<Building?>) {
    if (!autolink) return
    val valid = Boolf { other: Building? ->
      other != null && other.tile !== tile && other.block.connectedPower && other.power != null && (other.block.outputsPower || other.block.consumesPower || other.block is PowerNode) && overlaps(tile.x * Vars.tilesize + offset, tile.y * Vars.tilesize + offset, other.tile, laserRange * Vars.tilesize) && other.team === team && !graphs.contains(other.power.graph) && !insulated(tile, other.tile) && !(other is PowerNodeBuild && other.power.links.size >= (other.block as PowerNode).maxNodes) && !Structs.contains(Edges.getEdges(size)) { p: Point2? ->  //do not link to adjacent buildings
        val t = Vars.world.tile(tile.x + p!!.x, tile.y + p.y)
        t != null && t.build === other
      }
    }
