...
class TreeBlock(name: String) : EnvironmentBlock(name) {
    var shadowOffset = -4f
    ...
    override fun drawBase(tile: Tile) {
        val x = tile.worldx()
        val y = tile.worldy()
        val rot = Mathf.randomSeed(tile.pos().toLong(), 0, 4) * 90 + Mathf.sin(Time.time + x, 50f, 0.5f) + Mathf.sin(Time.time - y, 65f, 0.9f) + Mathf.sin(Time.time + y - x, 85f, 0.9f)
        val w = region.width * region.scl()
        val h = region.height * region.scl()
        val scl = 30f
        val mag = 0.2f
        val shad = if (variants == 0) customShadowRegion else variantShadowRegions[Mathf.randomSeed(tile.pos().toLong(), 0, max(0, variantShadowRegions.size - 1))]
        ...
        val reg = if (variants == 0) region else variantRegions[Mathf.randomSeed(tile.pos().toLong(), 0, max(0, variantRegions.size - 1))]
