...
open class StackConveyor(name: String) : IceBlock(name), Autotiler {
    companion object {
        const val stateMove = 0
        const val stateLoad = 1
        const val stateUnload = 2
    }
...
//parcel
    ...
    var edgeRegion: TextureRegion by LazyTextureSingleDelegate("${this.name}-edge")
    var stackRegion: TextureRegion by LazyTextureSingleDelegate("${this.name}-stack")
    ...
    var regions : Array<TextureRegion> by LazyTextureArrayDelegate(this.name, 3)
    ...
    override fun blends(tile: Tile, rotation: Int, otherx: Int, othery: Int, otherrot: Int, otherblock: Block): Boolean {
        ...
        if (b is StackConveyorBuild) {
            ...
            if (state == stateLoad) { //standard conveyor mode
                ...
                val b2 = notLookingAt(
                    tile, rotation, otherx, othery, otherrot, otherblock
                ) || (otherblock is StackConveyor && facing(
                    otherx, othery, otherrot, tile.x.toInt(), tile.y.toInt()
                ))
                val b3 = run {
                    val build = Vars.world.build(otherx, othery)

                    !(build is StackConveyorBuild && build.state == stateUnload)
                }
                val b4 = run {
                    val build = Vars.world.build(otherx, othery)
                    !(build is StackConveyorBuild && build.state == stateMove && !facing(
                        otherx, othery, otherrot, tile.x.toInt(), tile.y.toInt()
                    ))
                }
    ...
    override fun rotatedOutput(x: Int, y: Int): Boolean {
        ...
        if (tile is StackConveyorBuild) {
            return tile.state != stateUnload
        }
        return super.rotatedOutput(x, y)
    }

    open inner class StackConveyorBuild : IceBuild() {
        var state: Int = 0
        var blendprox: Int = 0
        var link: Int = -1
        ...
        override fun draw() {
            ...
            if (state == stateLoad) {
                for (i in 0..3) {
                    ...
                    if ((blendprox and (1 shl i)) != 0 && i != 0 && near != null && !near.block.squareSprite) {
                        Draw.rect(
                            sliced(regions[0], Autotiler.SliceMode.bottom), x + Geometry.d4x(dir) * Vars.tilesize * 0.75f, y + Geometry.d4y(dir) * Vars.tilesize * 0.75f, (dir * 90).toFloat()
                        )
                    }
                }
                ...
                if ((blendprox and (1)) != 0 && !front().block.squareSprite) {
                    Draw.rect(
                        sliced(regions[0], Autotiler.SliceMode.top), x + Geometry.d4x(rotation) * Vars.tilesize * 0.75f, y + Geometry.d4y(rotation) * Vars.tilesize * 0.75f, rotation * 90f
                    )
                }
            }
            ...
            if (glowRegion.found() && power != null && power.status > 0f) {
                ...
                Draw.rect(glowRegion, x, y, (rotation * 90).toFloat())
                Draw.blend()
                Draw.color()
                Draw.z(Layer.block - 0.1f)
            }
            ...
            if (drawLastItems) {
                //stack
                Draw.rect(
                    stackRegion, Tmp.v1.x, Tmp.v1.y, Mathf.lerp(a, b, Interp.smooth.apply(1f - Mathf.clamp(cooldown * 2, 0f, 1f)))
                )
                //item
                ...
                //stack
                val size = 2 + Vars.itemSize * Mathf.lerp(min((items.total().toFloat() / itemCapacity), 1.0f), 1f, 0.4f)
                Draw.rect(
                    stackRegion, Tmp.v1.x, Tmp.v1.y, size, size, Mathf.lerp(a, b, Interp.smooth.apply(1f - Mathf.clamp(cooldown * 2, 0f, 1f)))
                )
            }
        }
        ...
        override fun onProximityUpdate() {
            ...
            if (bits[0] == 0 && blends(tile, rotation, 0) && (!blends(
                    tile, rotation, 2
                ) || back() is StackConveyorBuild && (back() as StackConveyorBuild).state == stateUnload)
            ) {
            ...
            if (!outputRouter && front() !is StackConveyorBuild) state = stateUnload // a 0 that faces into none with a conveyor behind it
            if (!Vars.headless) {
                blendprox = 0

                for (i in 0..3) {
                    if (blends(tile, rotation, i) && (state != stateUnload || outputRouter || i == 0 || nearby(
                            Mathf.mod(rotation - i, 4)
                        ) is StackConveyorBuild)
                    ) {
            ...
            if (state == stateLoad) {
                for (near in proximity) {
                    if (near is StackConveyorBuild && near.front() === this) {
                        state = stateMove
                        break
                    }
                }
            ...
            if (state != lastState) {
                proxUpdating = true
                for (near in proximity) {
                    if (!(near is StackConveyorBuild && near.proxUpdating && near.state != stateUnload)) {
        ...
        override fun updateTile() {
            ...
            if (state == stateUnload) { //卸载
                ...
                if (state != stateLoad || (items.total() >= getMaximumAccepted(lastItem))) {
                    if (front() is StackConveyorBuild && (front() as StackConveyorBuild).team === team) {
                        val e = (front() as StackConveyorBuild)
        ...
        override fun overwrote(builds: Seq<Building>) {
            ...
            if (build is Conveyor.ConveyorBuild) {
