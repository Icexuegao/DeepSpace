...
object IceEffects {
  ...
  val jumpTrail: Effect = Effect(120f, 5000f, Cons { e: EffectContainer ->
    ...
    Draw.color(type.engineColor ?: e.color)
  ...
  fun shieldWave(unit: Unit, color: Color = IceColor.b4, range: Float = 50f): Effect {
    return Effect(30f) { e: EffectContainer ->
      ...
      IceDraw.light(
        unit.x, unit.y, Lines.circleVertices(70f), range, e.rotation, Color.clear, color, e::fout
      )
    }
  }
  ...
  fun getPhaseJump(x: Float, y: Float, rotate: Float, color: Color, team: Team, unit: UnitType, text: TextureRegion) {
    IEffect {
      ...
      set(5 * 60f) { e ->
        ...
        rand.setSeed(e.id.toLong())
        IceDraw.arc(
          e.x, e.y, 64f, 2f, 360 * e.fin(), e.fin() * max(
            100f, rand.random(360f)
          ) * 2.5f
        )
        IceDraw.arc(
          e.x, e.y, 48f, 2f, 360 * e.fin(), -e.fin() * max(
            100f, rand.random(360f)
          ) * 2.5f
        )
        val edge = 16f
  ...
  fun squareAngle(
    life: Float = 30f, amount: Int = 5, range: Float = 26f, color1: Color = Color.valueOf("ea8878"), color2: Color = Color.valueOf("c45f5f")
  ): Effect {
    return Effect(life) { e ->
  ...
  fun lightningShoot(
    life: Float = 12f, color: Color = IceColor.b4, amount: Int = 8, length: Float = 21f, range: Float = 30f
  ): Effect {
    return Effect(life) { e ->
      Draw.color(Color.white, color, e.fin())
      Lines.stroke(e.fout() * 1.2f + 0.5f)
      randLenVectors(
        e.id.toLong(), amount, length * e.finpow(), e.rotation, range
      ) { x: Float, y: Float ->
