                    ...
                    c.table(Styles.grayPanel) { b ->
                        ...
                        if (multipliers != null) {
                            b.add(
                                Strings.autoFixed(
                                    60f / (max(
                                        drillTime + drillMultiplier * block.itemDrop.hardness, drillTime
                                    ) / multipliers.get(block.itemDrop, 1f)) * size, 2
        ...
        return StatValue { table ->
            ...
            for (t in orderedKeys) {
                ...
                table.table(Styles.grayPanel) { bt ->
                    ...
                    if (!compact && t !is Turret) {
                        bt.table { title ->
                            ...
                            if (type.displayAmmoMultiplier && type.statLiquidConsumed > 0f) {
                                title.add(
                                    "[stat]" + StatValues.fixValue(
                                        type.statLiquidConsumed / type.ammoMultiplier * 60f
                    ...
                    if (type is ice.entities.bullet.base.BulletType) {
                        type.setStats(bt)
                    } else {
                        if (type.damage > 0 && (type.collides || type.splashDamage <= 0)) {
                            if (type.continuousDamage() > 0) {
                                bt.add(
                                    Core.bundle.format(
                                        "bullet.damage", type.continuousDamage()
                                    ) + StatUnit.perSecond.localized()
                                )
                            } else {
                        ...
                        if (type.buildingDamageMultiplier != 1f) {
                            sep(
                                bt, Core.bundle.format(
                                    "bullet.buildingdamage", ammoStat((type.buildingDamageMultiplier * 100 - 100).toInt().toFloat())
                                )
                            )
                        }
                        ...
                        if (type.rangeChange != 0f && !compact) {
                            sep(
                                bt, Core.bundle.format("bullet.range", ammoStat(type.rangeChange / Vars.tilesize))
                            )
                        }
                        ...
                        if (type.shieldDamageMultiplier != 1f) {
                            sep(
                                bt, Core.bundle.format(
                                    "bullet.shielddamage", ammoStat((type.shieldDamageMultiplier * 100 - 100).toInt().toFloat())
                                )
                            )
                        }
                        ...
                        if (type.splashDamage > 0) {
                            sep(
                                bt, Core.bundle.format(
                                    "bullet.splashdamage", type.splashDamage.toInt(), Strings.fixed(type.splashDamageRadius / Vars.tilesize, 1)
                                )
                            )
                        }

                        if (type.statLiquidConsumed <= 0f && !compact && !Mathf.equal(
                                type.ammoMultiplier, 1f
                        ...
                        if (type.healPercent > 0f) {
                            sep(
                                bt, Core.bundle.format("bullet.healpercent", Strings.autoFixed(type.healPercent, 2))
                            )
                        }

                        if (type.healAmount > 0f) {
                            sep(
                                bt, Core.bundle.format("bullet.healamount", Strings.autoFixed(type.healAmount, 2))
                            )
                        }

                        if (type.pierce || type.pierceCap != -1) {
                            sep(
                                bt, if (type.pierceCap == -1) "@bullet.infinitepierce" else Core.bundle.format(
                                    "bullet.pierce", type.pierceCap
                                )
                            )
                        }
                        ...
                        if (type.lightning > 0) {
                            sep(
                                bt, Core.bundle.format(
                                    "bullet.lightning", type.lightning, if (type.lightningDamage < 0) type.damage else type.lightningDamage
                                )
                            )
                        }
                        ...
                        if (type.maxDamageFraction > 0) {
                            sep(
                                bt, Core.bundle.format("bullet.maxdamagefraction", (type.maxDamageFraction * 100).toInt())
                            )
                        }
                        ...
                        if (type.suppressionRange > 0) {
                            sep(
                                bt, Core.bundle.format(
                                    "bullet.suppression", Strings.autoFixed(type.suppressionDuration / 60f, 2), Strings.fixed(type.suppressionRange / Vars.tilesize, 1)
                                )
                            )
                        }
                        ...
                        if (type.status !== StatusEffects.none) {
                            sep(
                                bt, (if (type.status.hasEmoji()) type.status.emoji() else "") + "[stat]" + type.status.localizedName + (if (type.status.reactive) "" else ("[lightgray] ~ [stat]" + Strings.autoFixed(
                                    type.statusDuration / 60f, 1
                ...
                }.padLeft(5f).padTop(5f).padBottom((if (compact) 0 else 5).toFloat()).growX().margin((if (compact) 0 else 10).toFloat())
                table.row()
            }
        }
    }
