...
private object DesktopAccessor: Accessor {
  ...
  init{
    ...
    exportNative = modelClass.getDeclaredMethod("addExports0",
      modelClass, String::class.java, modelClass
    )
    ...
    makeModuleOpen(
      ...
      Field::class.java.getPackage(),
      DesktopAccessor::class.java.module
    )
  ...
  fun makeModuleOpen(from: Module, pac: String, to: Module) {
    ...
    var opensMap = unsafe.getObjectVolatile(
      from,
      unsafe.objectFieldOffset(opensField)
    ) as MutableMap<String, MutableSet<Module>>?
    if (opensMap == null) {
      opensMap = HashMap<String, MutableSet<Module>>()
      unsafe.putObjectVolatile(from, unsafe.objectFieldOffset(opensField), opensMap)
    }

    var exportsMap = unsafe.getObjectVolatile(
      from,
      unsafe.objectFieldOffset(exportField)
    ) as MutableMap<String, MutableSet<Module>>?
    if (exportsMap == null) {
...
inline fun <U, reified T> KClass<*>.accessField(name: String) =
  this.java.getDeclaredField(name).also {
    if (!it.type.isAssignableFrom(T::class.java))
      throw IllegalArgumentException("field $it type is not instance of ${T::class.java}")
  }.let{
