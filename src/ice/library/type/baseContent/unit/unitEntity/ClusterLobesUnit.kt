package ice.library.type.baseContent.unit.unitEntity
...
class ClusterLobesUnit : UnitEntity() {
    var timer = 0
    ...
    val 激光 = object : LaserBulletType(100f) {
        init {
    ...
    override fun draw() {
        if (Vars.player.unit() == this) {
            Drawf.circles(x, y, 激光.length, Colors.b4)
        }
        //环线
        Draw.z(Layer.effect)
        ...
        //one 光环
        Lines.stroke(Mathf.sint(0.5f, 0.2f, 0f, 8f))
        ...
        Fill.circle(x + eye.x, y + eye.y, Mathf.sint(0.5f, 0.1f, 0f, 5f))
        //黑色环
    ...
    override fun update() {
        ...
        if (timer > 60 && target != null) {
            ...
            outsideRing.forEach {
                ...
                if (target!!.within(sub, 激光.length)) {
                    激光.create(this, sub.x, sub.y, Tmp.v2.set(it).add(x, y).sub(target).angle() + 180)
                }
            }
        }
        tris.forEach {
        ...
        Units.nearbyEnemies(team, x, y, 80f) {
            it.apply(IceStatus.电磁脉冲, 60 * 30f)
        }
        outsideRing.forEach {
