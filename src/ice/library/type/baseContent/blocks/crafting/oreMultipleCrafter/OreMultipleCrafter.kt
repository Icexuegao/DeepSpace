package ice.library.type.baseContent.blocks.crafting.oreMultipleCrafter
...
open class OreMultipleCrafter(name: String) : IceBlock(name) {
            ...
            table.table { pan ->
                ...
                oreFormula.forEach { ore ->
                    pan.iTableG { t ->
                        ...
                        t.iTableG { time ->
                            time.table {
                                it.image(Texs.time).size(21f).expandY().bottom()
                                it.add(Strings.format("@ @", ore.crftTime / 60f, "秒")).expandY().bottom()
                            }.grow().row()
                            time.table {
                                it.table { it1 ->
                                    it1.image(Texs.arrow).color(Colors.灰1).grow()
    ...
    override fun icons(): Array<TextureRegion> {
        return arrayOf(Core.atlas.find("$name-preview"))
    }

    inner class OreBlockBuild : Building() {
