...
import ice.library.type.meta.consumers.ConsumePowerMultiple
import ice.library.type.meta.stat.IceStatValues.formulas
...
class MultipleCrafter(name: String) : IceBlock(name) {
    ...
    override fun setBars() {
        ...
        if (consP) {
            addBar("power") { entity: MultipleCrafterBuilding ->
                Bar({
                    if ((entity.consPower != null && entity.consPower!!.buffered)) Core.bundle.format("bar.poweramount",
                        if (java.lang.Float.isNaN(
                                entity.power.status * entity.consPower!!.capacity)
                        ) "<ERROR>" else UI.formatAmount(
                            (entity.power.status * entity.consPower!!.capacity).toInt().toLong())) else Core.bundle.get(
                        "bar.power")
                }, { Pal.powerBar }, {
                    if (Mathf.zero(if (entity.consPower == null) 0f else entity.consPower!!.requestedPower(
    ...
    override fun init() {
        ...
        if (hasPower && consumesPower) {
            ...
            consPower = ConsumePowerMultiple(cs.toArray(csa))
        }
        hasConsumers = true
    }
                ...
                for ((index, form) in formulas.formulas.withIndex()) {
                    itable.button({ button ->
                        button.iTableGY { inputTable ->
                            inputTable.table(Texs.background42) {
                                form.inputs?.let { let ->
                                    let.forEach { cons ->
                                        cons.display(it)
                                    }
                        ...
                        button.iTableG { arrow ->
                            arrow.image(Texs.arrow1.apply {
                                texture.setFilter(Texture.TextureFilter.nearest)
                            }).size(180f, 80f)
                        }
                        button.iTableGY { out ->
                            out.table(Texs.background42) {
                        ...
                        up = Texs.background21
                        down = Texs.background22
                        checked = Texs.background22
                    }) {
                        setIndex(index)
