...
class IceDrill(name: String) : IceBlock(name) {
    ...
    init {
        ...
        drawers = IceDrawMulti(DrawDefault(), DrawRegion("-rotator", 3f, true), DrawRegionColor<IceDrillBuild>("-item"){
            it.dominantColor()
        })
    }
    ...
    override fun setBars() {
        ...
        addBar(
            ...
            Bar({
                Core.bundle.format(
                    "bar.drillspeed", Strings.fixed(60 / time * build.warmup() * build.timeScale(), 2)
                )
