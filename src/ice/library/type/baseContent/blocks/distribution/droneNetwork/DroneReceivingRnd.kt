...
class DroneReceivingRnd(name: String) : IceBlock(name) {
    ...
    init {
        ...
        drawers = IceDrawMulti(DrawRegion("-bottom"), DrawRegionColor<DroneReceivingRndBuild>("-center") {
            it.sortItem?.color
        }, DrawRegion("-top"))
    ...
    inner class DroneReceivingRndBuild : IceBuild() {
        ...
        override fun buildConfiguration(table: Table) {
            ItemSelection.buildTable(block, table, Vars.content.items(), ::sortItem, ::configure,true, 4, 6)
        }
