...
import ice.library.type.components.BuildInterface.BuildSeq
import ice.library.type.components.BuildInterface.BuildWorldLoadEndEvent
...
open class LinksBlock(name: String) : RangeBlock(name) {
    ...
    init {
        ...
        requirements(Category.effect, ItemStack.with(IceItems.石英玻璃, 10))
    }
    ...
    override fun setStats() {
        ...
        if (!allowLink.isEmpty) stats.add(IceStats.可连接建筑) { ta ->
            allowLink.forEach { b ->
                ta.image(b.uiIcon).size(32f).tooltip(b.localizedName)
            }
        }
    }
    ...
    open inner class LinksBlockBuildEnd : RangeBlockBuild(), BuildWorldLoadEndEvent, BuildSeq {
        override val builds = Seq<Building>()
        ...
        override fun onConfigureBuildTapped(other: Building): Boolean {
            ...
            if (builds.contains(other)) {
                ...
                if (addBuild(other)) { builds.addUnique(other) }
            }
            return false
        }
        ...
        override fun worldLoadEvent() {
            pos.forEach {

               if (addBuild(Vars.world.build(it))) { builds.addUnique(Vars.world.build(it)) }
            }
        }
    }
}