...
object IFiles : Load {
    ...
    private val modFile: Fi = ZipFi(Vars.modDirectory.list { it.name.contains(Ice.displayName) }.first())
    ...
    fun getModName(): String = Vars.mods.findMeta(modFile).internalName
    ...
    fun String.appendModName(): String {
        return getModName() + "-" + this
    }

    fun String.replaceModName(): String {
        return replace("${getModName()}-", "")
    }
    ...
    fun newCursor(filename: String): Graphics.Cursor {
        val fi = modFile.child("sprites").child("cursors").child("$filename.png")
        val p = Pixmap(fi)
