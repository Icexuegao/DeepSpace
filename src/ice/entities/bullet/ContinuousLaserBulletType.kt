package ice.entities.bullet

import arc.graphics.Color
import arc.graphics.g2d.Draw
import arc.graphics.g2d.Lines
import arc.math.Mathf
import arc.util.Time
import arc.util.Tmp
import mindustry.content.Fx
import mindustry.entities.Damage
import mindustry.gen.Bullet
import mindustry.graphics.Drawf
import kotlin.math.max

open class ContinuousLaserBulletType :ContinuousBulletType {
  var fadeTime: Float = 16f
  var lightStroke: Float = 40f
  var divisions: Int = 13
  var colors = arrayOf(Color.valueOf("ec745855"), Color.valueOf("ec7458aa"), Color.valueOf("ff9c5a"), Color.white)
  var strokeFrom: Float = 2f
  var strokeTo: Float = 0.5f
  var pointyScaling: Float = 0.75f
  var backLength: Float = 7f
  var frontLength: Float = 35f
  var width: Float = 9f
  var oscScl: Float = 0.8f
  var oscMag: Float = 1.5f

  constructor(damage: Float) {
    this.damage = damage
  }

  constructor()

  init {
    shake = 1f
    largeHit = true
    hitEffect = Fx.hitBeam
    hitSize = 4f
    drawSize = 420f
    lifetime = 16f
    hitColor = colors[2]
    incendAmount = 1
    incendSpread = 5f
    incendChance = 0.4f
    lightColor = Color.orange
    lightOpacity = 0.7f
  }

  override fun draw(b: Bullet) {
    val fout = Mathf.clamp(if (b.time > b.lifetime - fadeTime) 1f - (b.time - (lifetime - fadeTime)) / fadeTime else 1f)
    val realLength = Damage.findLength(b, length * fout, laserAbsorb, pierceCap)
    val rot = b.rotation()

    for(i in colors.indices) {
      Draw.color(Tmp.c1.set(colors[i]).mul(1f + Mathf.absin(Time.time, 1f, 0.1f)))

      val colorFin = i / (colors.size - 1).toFloat()
      val baseStroke = Mathf.lerp(strokeFrom, strokeTo, colorFin)
      val stroke = (width + Mathf.absin(Time.time, oscScl, oscMag)) * fout * baseStroke
      val ellipseLenScl = Mathf.lerp(1 - i / (colors.size).toFloat(), 1f, pointyScaling)

      Lines.stroke(stroke)
      Lines.lineAngle(b.x, b.y, rot, max(0f, realLength - frontLength), false)

      //back ellipse
      Drawf.flameFront(b.x, b.y, divisions, rot + 180f, backLength, stroke / 2f)

      //front ellipse
      Tmp.v1.trnsExact(rot, max(0f, realLength - frontLength))
      Drawf.flameFront(b.x + Tmp.v1.x, b.y + Tmp.v1.y, divisions, rot, frontLength * ellipseLenScl, stroke / 2f)
    }

    Tmp.v1.trns(b.rotation(), realLength * 1.1f)

    Drawf.light(b.x, b.y, b.x + Tmp.v1.x, b.y + Tmp.v1.y, lightStroke, lightColor, lightOpacity)
    Draw.reset()
  }

  override fun drawLight(b: Bullet?) {
    //no light drawn here
  }

  override fun currentLength(b: Bullet): Float {
    val fout = Mathf.clamp(if (b.time > b.lifetime - fadeTime) 1f - (b.time - (lifetime - fadeTime)) / fadeTime else 1f)
    return length * fout
  }
}
