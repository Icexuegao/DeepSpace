...
open class BlastLaser : EmpLightningBulletType() {
  ...
  override fun despawned(b: Bullet) {
    ...
    mindustry.entities.Effect.shake(despawnShake, despawnShake, b)
  }
  ...
  override fun update(b: Bullet) {
    ...
    if (b.timer(1, damageInterval)) {
      Damage.collideLaser(
        b, Mathf.len(b.aimX - b.x, b.aimY - b.y) * Mathf.clamp(b.time / blastDelay), true, false, -1
      )
    }
  }
    ...
    if (intervalBullet != null && b.time >= intervalDelay && b.timer.get(2, bulletInterval)) {
      ...
      for (i in 0..<intervalBullets) {
        intervalBullet.create(
          b, n.x, n.y, ang + Mathf.range(
            intervalRandomSpread
          ) + intervalAngle + ((i - (intervalBullets - 1f) / 2f) * intervalSpread), rand.random(0.4f, 1f), rand.random(0.8f, 1f)
        )
      }
    }
  }
    ...
    if (fragBullet != null && fragBullets > 0) {
      ...
      for (i in arr.indices) {
        if (arr[i] != null) {
          ...
          fragBullet.create(
            b.owner, b.team, x, y, Tmp.v1.angle(), fragBullet.damage, 1f, 1f, null, null, x + Tmp.v1.x, y + Tmp.v1.y
          )
        } else {
          val a = b.rotation() + Mathf.range(
            fragRandomSpread / 2
          ) + fragAngle + ((i - fragBullets / 2f) * fragSpread)

          Tmp.v1.set(fragBullet.range * Mathf.random(0.6f, 1f), 0f).setAngle(a)
          fragBullet.create(
            b.owner, b.team, x, y, a, fragBullet.damage, 1f, 1f, null, null, x + Tmp.v1.x, y + Tmp.v1.y
          )
        }
      }
    }
