...
import ice.vars.layout.Line;
import ice.vars.layout.Node;
import ice.vars.layout.TechTreeLayout;
...
import static ice.vars.layout.TechTreeLayout.checkCenter;


public class SglTechTreeDialog extends BaseDialog {
  public static ResearchManager researchManager=new ResearchManager();
  ...
  public void rebuildNodes(Planet planet){
    ...
    Seq<ResearchProject> projects = researchManager.listResearches(planet);
  ...
  static class TechNodeCard extends Table {
    ...
    private void rebuild(ResearchProject project) {
      ...
      if (!isMark) {
        Color matrixNet = Color.valueOf("D3FDFF");
        Color matrixNetDark = Color.valueOf("9ECBCD");
        NinePatchDrawable drawable = ((NinePatchDrawable) Tex.buttonSideRightOver).tint(matrixNet);
        Table tab = table(Tex.buttonSideRight, t -> {
          ...
          t.table(new BaseDrawable(){
            ...
            public void draw(float x, float y, float width, float height) {
              ...
              Draw.color(matrixNet, 0.6f*parentAlpha1);
              Lines.circle(x + width/2, y + height/2, rad);
              ...
              if (project.isProcessing()) {
                Lines.stroke(barStroke);
                Draw.color(matrixNetDark, parentAlpha);
                dashCircle(
              ...
              if (progress > 0) {
                Lines.stroke(barStroke);
                Draw.color(matrixNet, parentAlpha1);
                arc(
              ...
              if (subProgress > 0){
                Lines.stroke(frameStroke);
                Draw.color(matrixNet, 0.5f*parentAlpha1);
            ...
            if (isReveal) {
              ...
              img.image(new TextureRegion(Fonts.outline.getRegion().texture, g.u, g.v2, g.u2, g.v)).size(32).scaling(Scaling.fit).color(fexCrystal);
            }
          ...
          t.table(desc -> {
            desc.table(new BaseDrawable(){
              ...
              public void draw(float x, float y, float width, float height) {
                ...
                else Draw.color(fexCrystal, 0.3f*parentAlpha);

                Fill.rect(x + width/2, y + height/2, width, height);
              }
            }, prog -> {
              if (isReveal) {
                prog.image(Items.coal.uiIcon).scaling(Scaling.fit).size(22f).color(matrixNet);
  ...
  public static void arc(float x, float y, float radius, float scaleFactor, float innerAngel, float rotate){
    ...
    for (float ang = 0; ang <= innerAngel - step; ang += step){
      overed += step;
      Tmp.v1.set(radius, 0).setAngle(ang*sing + rotate);
    ...
    if (overed < innerAngel){
      Tmp.v1.set(radius, 0).setAngle(innerAngel*sing + rotate);
      Lines.linePoint(x + Tmp.v1.x, y + Tmp.v1.y);
    }
  ...
  public static void dashCircle(float x, float y, float radius, float scaleFactor, int dashes, float totalDashDeg, float rotate){
    ...
    Lines.beginLine();
    Tmp.v1.set(radius, 0).setAngle(rotate + 90);
    Lines.linePoint( Tmp.v1.x + x,  Tmp.v1.y + y);
    ...
    for(int i = 0; i < sides; i++){
      ...
      drawing = true;
      Tmp.v1.set(radius, 0).setAngle(rotate + per * (i + 1) + 90);
      float x1 =  Tmp.v1.x;
      float y1 =  Tmp.v1.y;
