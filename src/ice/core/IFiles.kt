...
object IFiles :Load {
  ...
  fun findPng(name: String): TextureAtlas.AtlasRegion = Core.atlas.find(name)
  fun findModPng(name: String): TextureAtlas.AtlasRegion = Core.atlas.find(name.appendModName())
  ...
  fun createNinePatch(name: String): Drawable {
    val region: TextureAtlas.AtlasRegion? = Core.atlas.find("${modWithClass.name}-$name")
