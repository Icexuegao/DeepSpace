...
open class AirSeaAmphibiousUnit(name: String) : SglUnitType<AirSeaAmphibiousUnit.AirSeaUnit>(name) {
                    ...
                    return object : FlyingAI() {
                        override fun updateMovement() {
                            ...
                            if ((core == null || !unit.within(core, unit.type.range * 0.5f))) {
                                ...
                                if (move) {
                                    moveTo(if (core != null) core else getClosestSpawner(), Vars.state.rules.dropZoneRadius + 130f)
                                }
                            }
    ...
    class AirSeaUnit : UnitWaterMove() {
        ...
        override fun add() {
            super.add()
            if (type is SglUnitType<*>) (type as SglUnitType<*>).init(this as SglUnitEntity)
            else throw RuntimeException("Unit type must be SglUnitType")
        }

        override fun read(read: Reads) {
            super.read(read)
          //  if (type is SglUnitType<*>) type.read(this, read, read.i())
            //else throw RuntimeException("Unit type must be SglUnitType")
        }
