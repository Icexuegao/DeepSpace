...
interface PayloadBuildComp : BuildCompBase {
 ...
 //   @Annotations.BindField(value = "payloads", initialize = "new singularity.world.modules.PayloadModule()")
    fun payloads(module: PayloadModule?) {
    }
    ...
    fun handleOutputPayload(): Float {
        if (outputting() != null) {
            ...
            outputting()!!.set(
                Mathf.approachDelta(outputting()!!.x(), building.x + outputVec.x, payloadSpeed()),
                Mathf.approachDelta(outputting()!!.y(), building.y + outputVec.y, payloadSpeed()),
                Angles.moveToward(outputting()!!.rotation(), Tmp.v1.set(outputVec).add(building).sub(outputting()).angle(), payloadRotateSpeed() * Time.delta)
            )
    ...
    fun outputtingOffset(): Vec2 {
        return tempVec.set(
            Angles.trnsx((building.rotation * 90).toFloat(), block!!.size * Vars.tilesize / 2f),
            Angles.trnsy((building.rotation * 90).toFloat(), block!!.size * Vars.tilesize / 2f)
        )
    }
    ...
    fun handleInputPayload(): Float {
        if (inputting() != null) {
            inputting()!!.set(
                Mathf.approachDelta(inputting()!!.x(), building.x, payloadSpeed()),
                Mathf.approachDelta(inputting()!!.y(), building.y, payloadSpeed()),
                Mathf.approachDelta(inputting()!!.rotation(), (building.rotation * 90).toFloat(), payloadRotateSpeed())
            )
 ...
 //   @MethodEntry(entryMethod = "write", paramTypes = "arc.util.io.Writes -> write")
   ...
   // @MethodEntry(entryMethod = "read", paramTypes = ["arc.util.io.Reads -> read", "byte -> revision"])
    fun readPayloads(read: Reads, revision: Byte) {
        ...
        inputting(Payload.read<Payload?>(read))
        outputting(Payload.read<Payload?>(read))
        stackAlpha(read.f())
