...
open class NuclearBlock(name: String) : SglBlock(name) {
  ...
  protected fun buildEnergySlider(sli: Table, min: Float, max: Float, yet: Floatp, slid: Floatc) {
    ...
    sli.button(Icon.rightOpen, Styles.clearNonei) { slid.get(Mathf.clamp(Mathf.pow(2, Mathf.log2(yet.get()).toInt() + 1).toFloat(), min, max)) }.size(32f)
    sli.add("").update { lable: Label? -> lable!!.setText(Mathf.round(yet.get()).toString() + "NF") }
  }
}