...
open class NuclearNode(name: String) : NuclearBlock(name) {
  ...
  fun getPotentialLink(tile: Tile, team: Team?, cons: Cons<NuclearEnergyBuildComp?>) {
    val valid = Boolf { other: NuclearEnergyBuildComp? ->
      other != null && other.building.tile !== tile && other.energy() != null && other.consumeEnergy() && inRange(tile, other.building.tile, linkRange * Vars.tilesize) && other.building.team === team && !Structs.contains(Edges.getEdges(size)) { p: Point2? ->  //do not link to adjacent buildings
