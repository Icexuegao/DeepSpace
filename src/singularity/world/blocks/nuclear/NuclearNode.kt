...
open class NuclearNode(name: String) : NuclearBlock(name) {
  ...
  override fun appliedConfig() {
    ...
    config(Point2::class.java, Cons2 { entity: NuclearNodeBuild, value: Point2 ->

      val tile = Vars.world.tile(value.x, value.y) ?: return@Cons2
      if ((tile.build !is NuclearEnergyBuildComp) || !(tile.build as NuclearEnergyBuildComp).hasEnergy()) return@Cons2

      value.x = tile.x.toInt()
      value.y = tile.y.toInt()
      val build = tile.build as NuclearEnergyBuildComp

      if (entity.linked.contains(value.pack())) {
        entity.deLink(build)
      } else {
        if (entity.linksCount() >= maxLinks || !canLink(entity, build)) return@Cons2
        entity.link(build)
      }
    })
  }
  ...
  override fun drawPlanConfigTop(req: BuildPlan, list: Eachable<BuildPlan?>) {
    if (req.config is Array<*> && (req.config as Array<*>).isArrayOf<Point2>()) {
      val config = req.config as Array<Point2>
      for (point in config) {
        val px: Int = req.x + point.x
        val py: Int = req.y + point.y
        otherReq = null
        list.each(Cons { other: BuildPlan? ->
          if (other!!.block != null && (px >= other.x - ((other.block.size - 1) / 2) && py >= other.y - ((other.block.size - 1) / 2) && px <= other.x + other.block.size / 2 && py <= other.y + other.block.size / 2) && other !== req && other.block is NuclearBlock) {
            otherReq = other
          }
        })
  ...
  override fun drawPlace(x: Int, y: Int, rotation: Int, valid: Boolean) {
    ...
    if (lastPlaced != null && lastPlaced!!.isAdded && inRange(tile, lastPlaced!!.tile, linkRange * Vars.tilesize) && inRange(lastPlaced!!.tile, tile, (lastPlaced!!.block as NuclearNode).linkRange * Vars.tilesize) && lastPlaced!!.linksCount() < ((lastPlaced!!.block) as NuclearNode).maxLinks) {
      setLaserColor()
      Draw.alpha(0.5f)
  ...
  fun getPotentialLink(tile: Tile, team: Team, cons: Cons<NuclearEnergyBuildComp>) {
    val valid = Boolf { other: NuclearEnergyBuildComp ->
      other.building.tile !== tile && other.consumeEnergy() && inRange(tile, other.building.tile, linkRange * Vars.tilesize) && other.building.team === team && !Structs.contains(Edges.getEdges(size)) { p: Point2? ->  //请勿链接至相邻建筑
    ...
    Geometry.circle(tile.x.toInt(), tile.y.toInt(), (linkRange + 2).toInt(), Intc2 { x: Int, y: Int ->
      ...
      if (!tempNuclearEntity.contains(other) && valid.get(other)) {
        tempNuclearEntity.add(other)
      }
    })
    ...
    returnInt = 0
    tempNuclearEntity.forEach { e: NuclearEnergyBuildComp? ->
  ...
  open inner class NuclearNodeBuild : SglBuilding() {
    ...
    var linked = IntSeq(false, 16)
    var linkThis = IntSeq(false, 16)
    ...
    override fun playerPlaced(config: Any?) {
      ...
      if (lastPlaced != null && lastPlaced!!.isAdded) {
        if (canLink(this, lastPlaced!!) && !linked.contains(lastPlaced!!.pos())) {
    ...
    override fun onConfigureBuildTapped(other: Building): Boolean {
      ...
      if (other === this) {
        if (linked.size > 0) {
          ...
          getPotentialLink(tile, team) { e: NuclearEnergyBuildComp ->
            configure(Point2.unpack(e.building.pos()))
          }
        }
        return false
      }
    ...
    override fun updateTile() {
      ...
      dumpEnergy(getNodeDumps() as Seq<NuclearEnergyBuildComp?>)
    }
    ...
    override fun draw() {
      ...
      for (i in 0..<linked.size) {
        val pos = linked.get(i)
        val entity = Vars.world.build(pos) ?: continue
        ...
        Draw.draw(Draw.z()) {
          ...
          for (ig in 0..2) {
            ...
            MathRenderer.drawSin(
              x, y, tx, ty, rand.random(linkLeaserStroke * 1.2f, linkLeaserStroke * 1.8f) * alpha * (0.9f + 0.1f * Mathf.sin(Time.time / rand.random(10f, 15f))), rand.random(360f, 720f), -rand.random(5f, 8f) * Time.time
            )
          }
          Draw.reset()
        }
    ...
    fun arrow(x: Float, y: Float, x2: Float, y2: Float, length: Float, radius: Float, color: Color) {
      val angle = Angles.angle(x, y, x2, y2)
      Tmp.v1.set(x2, y2).sub(x, y).limit(length)
      val vx = Tmp.v1.x + x
      val vy = Tmp.v1.y + y
    ...
    fun lineGray(color: Color, x: Float, y: Float, x2: Float, y2: Float) {
      Lines.stroke(3f)
    ...
    fun line(color: Color, x: Float, y: Float, x2: Float, y2: Float) {
    ...
    override fun drawConfigure() {
      Drawf.circles(x, y, tile.block().size * Vars.tilesize / 2f + 1f + Mathf.absin(Time.time, 4f, 1f), IceColor.b4)
      ...
      for (i in 0..<linked.size) {
        ...
        arrowGray(x, y, link.x, link.y, len * (Time.time % 120) / 120, linkLeaserStroke * 2, linkColor)
        lineGray(linkColor, x, y, link.x, link.y)

        arrow(x, y, link.x, link.y, len * (Time.time % 120) / 120, linkLeaserStroke * 2, linkColor)
        line(linkColor, x, y, link.x, link.y)
        Drawf.square(link.x, link.y, link.block.size * Vars.tilesize / 1.8f + Mathf.absin(4f, 2f), Pal.place)
      }
      ...
      for (i in 0..<linkThis.size) {
        ...
        Drawf.dashLine(IceColor.y1, x, y, other.x, other.y)
        Drawf.arrow(other.x, other.y, x, y, len * (Time.time % 120) / 120, linkLeaserStroke * 2, IceColor.y1)

        Drawf.square(other.x, other.y, other.block.size * Vars.tilesize / 1.8f + Mathf.absin(4f, 2f), IceColor.y1)
      }
