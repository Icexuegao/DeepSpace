...
open class TokamakCore(name: String) : NormalCrafter(name), SpliceBlockComp {
    ...
    companion object {
      ...
      //  const val VALID: String = "valid"
        private val model: ParticleModel = MultiParticleModel(
            ...
            object : ParticleModel() {
                val cacheVecs: Seq<Vec2?> = Seq<Vec2?>()
                var cursor: Int = -1

                init {
                    //  Sgl.ui.debugInfos.addMonitor("vecCursor", Prov { cursor })
                }
...
//        const val : String = "totalLiquidCapacity"
        ...
        override fun canChain(other: ChainsBuildComp): Boolean {
            return chainable(other.chainsBlock) && other is TokamakOrbitBuild
                    && (relativeTo(other).toInt() == other.rotation || relativeTo(other).toInt() == (other.rotation + 2) % 4)
        }
