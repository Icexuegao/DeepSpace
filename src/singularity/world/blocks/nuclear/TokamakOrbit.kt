...
open class TokamakOrbit(name: String) :SglBlock(name), SpliceBlockComp {
  ...
  init {
    ...
    drawers = DrawMultiSgl(object :DrawRegionDynamic<TokamakOrbitBuild>("_corner_bottom") {
      init {
        alpha = Floatf { e -> if (e.isCorner) 1f else 0f }
        rotation = Floatf { e -> (e.rotation * 90).toFloat() }
        makeIcon = false
      }
    }, object :DrawRegionDynamic<TokamakOrbitBuild>("_bottom") {
      init {
        alpha = Floatf { e -> if (e.isCorner) 0f else 1f }
        rotation = Floatf { e -> e.rotation * 90f }
        makeIcon = true
        drawPlan = true
      }
    }, object :DrawBlock() {
      var light: TextureRegion by LazyTextureSingleDelegate(this@TokamakOrbit.name + "_light")
      var cornerLight: TextureRegion by LazyTextureSingleDelegate(this@TokamakOrbit.name + "_corner_light")
      ...
      var conduit: TextureRegion by LazyTextureSingleDelegate(this@TokamakOrbit.name)
      var arrow: TextureRegion by LazyTextureSingleDelegate(this@TokamakOrbit.name + "_arrow")
      ...
      override fun icons(block: Block) = arrayOf(conduit, arrow)
    }, DrawRegionDynamic<TokamakOrbitBuild>("_corner").apply {
        alpha = Floatf { e -> if (e.isCorner) 1f else 0f }
        makeIcon = false
      }
  ...
  inner class TokamakOrbitBuild :SglBuilding(), SpliceBuildComp {
    ...
    override fun canChain(other: ChainsBuildComp): Boolean {
      return chainable(other.chainsBlock) && ((other is TokamakCoreBuild && (other.relativeTo(this)
        .toInt() == rotation || relativeTo(other).toInt() == rotation)) || (other.tileX() == tileX() || other.tileY() == tileY()) && (relativeTo(
        other.building
      ).toInt() == rotation || other.building.relativeTo(
        this
      ).toInt() == other.building.rotation))
    }
    ...
    override fun onProximityUpdate() {
      ...
      for(comp in chainBuilds()) {
        if ((comp is TokamakOrbitBuild || comp is TokamakCoreBuild)) {
          ...
          } else if ((comp is TokamakOrbitBuild && comp.relativeTo(this)
              .toInt() == comp.rotation) || (comp is TokamakCoreBuild && comp.relativeTo(this).toInt() == rotation)
          ) {
      ...
      isCorner =
        facingThis.size > 1 || facingThis.size == 1 && (facingThis.get(0) !is TokamakCoreBuild) && facingThis.get(0)!!.relativeTo(this)
          .toInt() != rotation
      updateRegionBit()
    }
