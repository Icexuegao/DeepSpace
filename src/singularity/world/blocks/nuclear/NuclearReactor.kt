...
open class NuclearReactor(name: String) : NormalCrafter(name) {
  ...
  fun addCoolant(consHeat: Float) {
    val cons = newOptionalConsume({ e: ConsumerBuildComp, c: BaseConsumers ->
      ...
      s!!.add(SglStat.effect) { t: Table? ->
        t!!.row()
        t.add(
          (Core.bundle.get("misc.absorbHeat") + ": " + Strings.autoFixed(consHeat * 60, 2) + SglStatUnit.heat.localized() + Core.bundle.get(
            "misc.perSecond"
          ))
        )
      }
    ...
    cons?.setConsDelta { e: ConsumerBuildComp ->
      e.building.delta()
  ...
  override fun setStats() {
    ...
    stats.add(
      SglStat.heatProduct,
      Strings.autoFixed(productHeat * 60, 2) + SglStatUnit.heat.localized() + Core.bundle.get("misc.perSecond")
    )
  ...
  override fun setBars() {
    super.setBars()
    addBar("efficiency") { e: NuclearReactorBuild ->
      Bar(
    ...
    addBar("heat") { e: NuclearReactorBuild ->
      Bar(
        { IceStats.热量.localizedName + if (SettingValue.启用调试模式) ": ${e.heat.toStringi(1)}" else "" }, Pal::lightOrange
      ) { e.heat / maxHeat }
    }
  }
  ...
  inner class NuclearReactorBuild : NormalCrafterBuild() {
    ...
    override fun consEfficiency(): Float {
      return fuelItemsTotal() / itemCapacity * super.consEfficiency() * (1 - Mathf.clamp(
        (items.get(IItems.核废料) - itemCapacity / 3f) / (itemCapacity),
        0f,
        1f
      ))
    }
