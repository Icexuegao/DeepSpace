...
open class EnergyBuffer(name: String) : NuclearNode(name) {
    ...
    override fun appliedConfig() {
        ...
        config(Array<Any>::class.java, Cons2 { b: EnergyBufferBuild, arr: Array<Any> ->
            if (arr!![0] is Boolean) {
    ...
    inner class EnergyBufferBuild : NuclearNodeBuild() {
        ...
        override fun buildConfiguration(table: Table) {
            ...
            table.table(Cons { t: Table? ->
                ...
                t.table(Tex.pane, Cons { ta: Table? ->
                    ta!!.image(Icon.download).size(40f).get().setScaling(Scaling.fit)
                    buildEnergySlider(ta, minPotential, maxPotential, Floatp { input }, Floatc { f: Float -> configure(arrayOf<Any>(false, f)) })
                    ta.row()
                    ta.image(Icon.upload).size(40f).get().setScaling(Scaling.fit)
                    buildEnergySlider(ta, minPotential, maxPotential, Floatp { output }, Floatc { f: Float -> configure(arrayOf<Any>(true, f)) })
                })
        ...
        override val inputPotential=min(input, getEnergy())
        override val outputPotential= min(output, getEnergy())
