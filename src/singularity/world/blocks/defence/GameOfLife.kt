...
open class GameOfLife(name: String) : SglBlock(name) {
    ...
    val launchCons: SglConsumers = object : SglConsumers(false) {
        init {
            setConsDelta(Floatf { e: GameOfLifeBuild -> e.launchEff * Time.delta })
        }

        public override fun <T : BaseConsume<out ConsumerBuildComp>> add(consume: T): T {
    ...
    init {
        ...
        config<Point2?, GameOfLifeBuild?>(Point2::class.java, Cons2 { e: GameOfLifeBuild?, p: Point2? ->
            val cell = e!!.grid!!.get(p!!.x, p.y)
            if (cell == null) throw RuntimeException("position out of bound")
            if (cell.isLife) {
                cell.kill()
            } else cell.born()
        })

        config<Int?, GameOfLifeBuild?>(Int::class.java, Cons2 { e: GameOfLifeBuild?, i: Int? ->
            if (i == 0) {
                e!!.activity = false
                e.editing = true
                e.grid!!.resetYears()
        ...
        config<IntSeq?, GameOfLifeBuild?>(IntSeq::class.java, Cons2 { e: GameOfLifeBuild?, seq: IntSeq? ->
            for (cell in e!!.grid!!) {
                cell.kill()
            }
            for (i in 0..<seq!!.size) {
    ...
    public override fun load() {
        super.load()
        cellRegion = arrayOfNulls<TextureRegion>(maxCellYears + 1 + (if (cellSenescence) 1 else 0))
    ...
    override fun canPlaceOn(tile: Tile, team: Team?, rotation: Int): Boolean {
        return Vars.indexer.findTile(team, tile.worldx(), tile.worldy(), cellSize * gridSize * Mathf.sqrt2 / 2, Boolf { b: Building? -> b!!.block === this }) == null
    }
    ...
    public override fun setBars() {
        ...
        addBar<GameOfLifeBuild?>("cells", Func { e: GameOfLifeBuild? ->
            Bar(
                Prov { Core.bundle.format("bar.cellCount", e!!.lifeCells) },
                Prov { Pal.items },
                Floatp { 1f }
            )
        })
        addBar<GameOfLifeBuild?>("launch", Func { e: GameOfLifeBuild? ->
            Bar(
                Prov { Core.bundle.format("bar.launchProgress", e!!.warmup * 100) },
                Prov { Pal.powerBar },
                Floatp { e!!.warmup }
            )
        })
    ...
    public override fun setStats() {
        ...
        stats.add(SglStat.recipes, StatValue { t: Table? ->
            ...
            for (i in 0..<consumers.size) {
                ...
                t.table(SglDrawConst.grayUIAlpha, Cons { ta: Table? -> ta!!.add<Table?>(details).pad(4f) })
                t.row()
            }
        })
        ...
        stats.add(SglStat.launchConsume, StatValue { t: Table? ->
            val stat = Stats()
        ...
        stats.add(SglStat.effect, StatValue { t: Table? ->
            t!!.row()
            for (i in 0..maxCellYears) {
                ...
                t.table(SglDrawConst.grayUIAlpha, Cons { item: Table? ->
    ...
    fun addDeathTrigger(minYears: Int, maxYears: Int, caller: CellCaller) {
        deathTriggers.add(object : CellCaller(minYears, maxYears) {
            public override fun call(build: GameOfLifeBuild, cell: Cell?, x: Float, y: Float) {
                if (valid(cell!!.years)) caller.call(build, cell, x, y)
            }

            public override fun setStats(stats: Stats) {
    ...
    inner class GameOfLifeBuild : SglBuilding() {
        ...
        override fun buildConfiguration(table: Table) {
            table.button(Icon.pencil, Styles.cleari, Runnable {
                table.clearChildren()
                configure(0)
            }).size(50f)

            table.button(Icon.play, Styles.cleari, Runnable { configure(1) })
                .update(Cons { b: ImageButton? -> b!!.getStyle().imageUp = if (activity) Icon.pause else Icon.play }).size(50f)

            table.button(Icon.cancel, Styles.cleari, Runnable { configure(2) }).size(50f)
        }
        ...
        public override fun consumeValid(): Boolean {
        ...
        fun launched() {
            ...
            for (cell in grid!!) {
                ...
                val cx = v.x
                val cy = v.y
                Time.run(dis * 5, Runnable { SglFx.cellScan.at(cx, cy, (if (rot45) 45 else 0).toFloat(), gridColor, block) })
            }
            for (cons in launchCons.all()) {
        ...
        fun untrns(x: Float, y: Float): Cell {
            ...
            x = tmp.x
            y = tmp.y
            val dx = Math.round(x / cellSize + grid!!.offset)
    ...
    abstract class CellCaller {
        ...
        constructor() {
            minYears = Int.Companion.MIN_VALUE
            maxYears = Int.Companion.MAX_VALUE
        }
    ...
    companion object {
        ...
        fun shootBullet(bullet: BulletType, pattern: ShootPattern): CellCaller {
            return object : CellCaller() {
                public override fun call(build: GameOfLifeBuild, cell: Cell?, x: Float, y: Float) {
                    pattern.shoot(0, BulletHandler { offX: Float, offY: Float, rotation: Float, delay: Float, move: Mover? ->
                        bullet.create(build, build!!.team, x + offX, y + offY, rotation, bullet.damage, 1f, 1f, null, move)
                    })
                }

                public override fun setStats(stats: Stats) {
                    stats.add(Stat.ammo, StatValue { t: Table? ->
                        t!!.table(Cons { bt: Table? ->
                            bt!!.defaults().left()
                            if (pattern.shots > 1) {
