...
open class PhasedRadar(name: String) : SglBlock(name), SpliceBlockComp {
    ...
    inner class PhasedRadarBuild : SglBuilding(), SpliceBuildComp {
        ...
        override fun getPowerConnections(out: Seq<Building?>?): Seq<Building?>? {
                ...
                if (timer(timeId, scanTime)) {
                    for (unit in Groups.unit) {
                        ...
                        if (unit.team !== team && unit.isFlying()
                            && ((Mathf.len(unit.x - centerPos.x, unit.y - centerPos.y) < range * Vars.tilesize).also { lenValid = it })
                            && !locking.contains(unit) && locking.size < min(chains!!.container.all.size, 10)
                        ) {
                            locking.add(unit)
        ...
        override fun chainsFlowed(old: ChainsContainer?) {
            val statDisplay: PhasedRadarBuild
            if ((chains!!.container.BUILD.also { statDisplay = it }) !== this) {
                if (statDisplay.y >= y && statDisplay.x <= building.x) {
                    chains!!.container.BUILD = this
                    centerPos.set(chains!!.container.minX().toFloat(), chains!!.container.minY().toFloat())
                        .scl(Vars.tilesize.toFloat())
                        .add(chains!!.container.width() / 2f * Vars.tilesize, chains!!.container.height() / 2f * Vars.tilesize)
                }
            }
        }
        ...
        override fun drawSelect() {
            ...
            Draw.color(Pal.placing)
            Draw.alpha(0.4f)
            val b: PhasedRadarBuild = chains!!.BUILD
            ...
            SglDraw.gradientLine(
                drawX + Tmp.v2.x * 1.75f, drawY + Tmp.v2.y * 3,
                drawX + Tmp.v2.x * 4.25f + dx, drawY + Tmp.v2.y * 4.25f + dy,
                Pal.placing, Tmp.c1.set(Pal.placing).a(0f), 1
            )
