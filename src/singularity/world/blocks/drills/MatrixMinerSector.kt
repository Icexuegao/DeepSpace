...
open class MatrixMinerSector(name: String) : MatrixMinerPlugin(name) {
  ...
  init {
    buildType = Prov(::MatrixMinerSectorBuild)
  }
  ...
  inner class MatrixMinerSectorBuild : MatrixMinerPluginBuild() {
    ...
    override var owner: MatrixMinerBuild? = null
      set(value) {
        field = value
        side = field!!.relativeTo(this)
      }
    ...
    override fun drawConfigure() {
      ...
      Tmp.v2.set(Tmp.v1).rotate90(1)
      Fill.tri(
        owner!!.x, owner!!.y, owner!!.x + Tmp.v1.x + Tmp.v2.x, owner!!.y + Tmp.v1.y + Tmp.v2.y, owner!!.x + Tmp.v1.x - Tmp.v2.x, owner!!.y + Tmp.v1.y - Tmp.v2.y
      )
    }
    ...
    public override fun updatePlugin(owner: MatrixMinerBuild?) {
      if (nextPos == null) {
        ...
        while (c < len) {
          ...
          if (tile == null || (!owner.pierce && tile.build != null) || abs(tile.x - owner.tileX()) > owner.drillRange / 2 || abs(tile.y - owner.tileY()) > owner.drillRange / 2 || !owner.angleValid(Angles.angle(tile.worldx() - owner.x, tile.worldy() - owner.y))) continue
          val ore: Item?
    ...
    fun drawDrill() {
      ...
      Draw.draw(Draw.z(), Runnable {
        ...
        for (i in 0..3) {
          MathRenderer.drawSin(
            x, y, rand.random(2f, 3.4f), drillPos!!.x, drillPos!!.y, 1.6f, rand.random(500f, 800f), rand.random(2f, 3.6f) * Time.time
          )
        }
      })
      ...
      SglDraw.drawCornerTri(
        drillPos!!.x, drillPos!!.y, 24 * warmup, 5 * warmup, Time.time * 1.5f, true
      )
    }
