...
open class MultLiquidBlock(name: String?) :LiquidBlock(name) {
  ...
  init {
    displayFlow = false
    buildType = Prov(::MultLiquidBuild)
  }
  ...
  companion object {
    ...
    fun groupName(i: Int): String {
      if (groupName.size <= i) {
        groupName = groupName.copyOf(i + 1)
  ...
  open inner class MultLiquidBuild :Building(), ReplaceBuildComp {
    var liquidsBuffer: Array<ClusterLiquidModule> = Array(conduitAmount){
      ClusterLiquidModule()
    }
    ...
    override fun displayBars(table: Table) {
      ...
      for(i in liquidsBuffer.indices) {
        ...
        table.add(Bar({
          if (current.smoothCurrent <= 0.001f) Core.bundle.get("bar.liquid") + " #" + fi else current.current().localizedName + "     " + (if (current.getFlowRate(
              current.current()
            ) >= 0
          ) Strings.autoFixed(current.getFlowRate(current.current()), 0) else "...") + Core.bundle.get("misc.perSecond")
        }, { current.current().barColor() }, { current.smoothCurrent }))
        table.row()
      }
    }
      ...
      if (flowTimer.get(1, 20f)) {
        if (flow == null) {
          if (cacheFlow == null || cacheFlow!!.size != liquids.size) {
            cacheFlow = arrayOfNulls(liquids.size)
