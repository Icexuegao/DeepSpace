...
open class LiquidUnloader(name: String?) : Block(name) {
    ...
    init {
        ...
        config<Liquid?, LiquidUnloadedBuild?>(Liquid::class.java, Cons2 { tile: LiquidUnloadedBuild?, l: Liquid? -> tile!!.current = l })
        configClear<LiquidUnloadedBuild?>(Cons { tile: LiquidUnloadedBuild? -> tile!!.current = null })
    }
    ...
    inner class LiquidUnloadedBuild : Building(), Takeable {
        ...
        override fun updateTile() {
            val next = getNext("liquidsPeek", Boolf { e: Building? -> e!!.block.hasLiquids && e.canUnload() })
            if (next == null) return

            if (next.liquids != null) {
                val dmp = LiquidConsumer { l: Liquid?, a: Float ->
                    var dump = getNext("liquids", Boolf { e: Building? ->
        ...
        override fun buildConfiguration(table: Table) {
            ItemSelection.buildTable<Liquid?>(table, Vars.content.liquids(), Prov { current }, Cons { value: Liquid? -> this.configure(value) })
        }
