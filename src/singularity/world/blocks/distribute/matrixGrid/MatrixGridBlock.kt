...
open class MatrixGridBlock(name: String) : DistNetBlock(name), DistMatrixUnitComp {
  ...
  init {
    ...
    this.hasItems = this.hasLiquids
    this.outputsLiquid = false
    this.outputItems = this.outputsLiquid
  ...
  override fun setStats() {
    super.setStats()
    this.stats.add(SglStat.bufferSize, StatValue { t: Table? ->
    ...
    public override fun updateTile() {
      ...
      if (this.gridValid()) {
        for (value in GridChildType.entries) {
          for (entry in this.matrixGrid().get<Any?>(value, Boolf2 { bx: Any?, cx: TargetConfigure? -> true })) {
        ...
        for (request in requests()) {
          ...
          request.update(
            { t: Boolp -> (handler as RequestHandlers.RequestHandler<DistRequestBase>).preCallBack(this, request as DistRequestBase, t) },
            { t: Boolp -> (handler as RequestHandlers.RequestHandler<DistRequestBase>).callBack(this, request as DistRequestBase, t) },
            { t: Boolp -> (handler as RequestHandlers.RequestHandler<DistRequestBase>).afterCallBack(this, request as DistRequestBase, t) }
          )
        }
      }
