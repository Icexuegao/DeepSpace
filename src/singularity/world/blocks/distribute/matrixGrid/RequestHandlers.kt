...
class RequestHandlers {
    ...
    interface RequestHandler<R : DistRequestBase> {
         ...
         */
        fun makeRequest(sender: DistMatrixUnitBuildComp): DistRequestBase?
    ...
    abstract class AbstractItemRequestHandler {
        ...
        open fun reset(sender: DistMatrixUnitBuildComp) {
            items.clear()
            val coreBuff = sender.distributor()!!.core().getBuffer(DistBufferType.itemBuffer)!!
     ...
     abstract class AbstractLiquidRequestHandler {
        ...
        protected var total: Float = 0f
         ...
         open  fun reset(sender: DistMatrixUnitBuildComp) {
            liquids.clear()
            total = 0f
            val coreBuff = sender.distributor()!!.core().getBuffer(DistBufferType.liquidBuffer)!!
        ...
        open fun addParseConfig(cfg: TargetConfigure, type: GridChildType) {
  ...
  * */
    ...
    class ReadItemRequestHandler : AbstractItemRequestHandler(), RequestHandler<ReadItemsRequest> {
        ...
        override fun callBack(sender: DistMatrixUnitBuildComp, request: ReadItemsRequest, task: Boolp): Boolean {
            Arrays.fill(tmp, 0)
            tmpItems.clear()
            val buffer = sender.getBuffer(DistBufferType.itemBuffer)
            ...
            for (entry in sender.matrixGrid()!!.get<Any?>(GridChildType.output, Boolf2 { e: Any?, c: TargetConfigure? -> e is GenericIOPPointBuild })) {
                ...
                for (item in ioPoint.config!!.get(GridChildType.output, ContentType.item)) {
                    if (buffer.get((item as Item?)!!) > 0 && ioPoint.acceptItemOut(sender.building, item as Item)) {
            ...
            val coreBuffer = sender.distributor()!!.core().getBuffer(DistBufferType.itemBuffer)!!
  ...
  * */
    class AcceptLiquidRequestHandler : AbstractLiquidRequestHandler(), RequestHandler<PutLiquidsRequest> {
        ...
        override fun makeRequest(sender: DistMatrixUnitBuildComp): PutLiquidsRequest? {
            handleToOne()
            return if (liquids.isEmpty()) null else PutLiquidsRequest(sender, sender.getBuffer(DistBufferType.liquidBuffer)!!, Seq<LiquidStack?>(liquids.values().toArray()) as Seq<LiquidStack>?)
        }
    }
    ...
    class ReadLiquidRequestHandler : AbstractLiquidRequestHandler(), RequestHandler<ReadLiquidsRequest> {
        ...
        override fun makeRequest(sender: DistMatrixUnitBuildComp): ReadLiquidsRequest? {
            handleToOne()
            val seq = Seq(liquids.values().toArray())
        ...
        override fun callBack(sender: DistMatrixUnitBuildComp, request: ReadLiquidsRequest, task: Boolp): Boolean {
            Arrays.fill(tmp, 0f)
            tmpLiquids.clear()
            val buffer = sender.getBuffer(DistBufferType.liquidBuffer)
            for (packet in buffer!!) {
                tmp[packet.id()] = packet.amount()
            ...
            for (entry in sender.matrixGrid()!!.get<Any?>(GridChildType.output, Boolf2 { e: Any?, c: TargetConfigure? -> e is GenericIOPPointBuild })) {
                ...
                for (liquid in ioPoint.config!!.get(GridChildType.output, ContentType.liquid)) {
                    ...
                    if (buffer.get(li) > 0 && ioPoint.acceptLiquidOut(sender.building, li)) {
                        val all = sender.matrixGrid()!!.get<Any?>(GridChildType.output, Boolf2 { e: Any?, c: TargetConfigure? -> c!!.get(GridChildType.output, liquid) }).size
            ...
            val coreBuffer = sender.distributor()!!.core().getBuffer(DistBufferType.liquidBuffer)!!
            for (id in tmp.indices) {
