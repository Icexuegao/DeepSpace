...
open class ItemNode(name: String) : SglBlock(name) {
  ...
  public override fun parseConfigObjects(b: SglBuilding?, obj: Any?) {
    ...
    if (obj is TargetConfigure) {
      val c = obj
      e.config = if (c.isClear) null else c
      e.link = if (c.offsetPos != 0) Point2.unpack(c.offsetPos).add(e.tileX(), e.tileY()).pack() else e.link
    }
  }
  ...
  public override fun setBars() {
    ...
    this.addBar<Building?>("items", Func { entity: Building? -> Bar(Prov { Core.bundle.format("bar.items", *arrayOf<Any>(entity!!.items.total())) }, Prov { Pal.items }, Floatp { entity!!.items.total().toFloat() / this.maxItemCapacity.toFloat() }) })
  }
  ...
  fun linkValid(tile: Tile?, other: Tile?, checkDouble: Boolean = true): Boolean {
    if (other != null && tile != null && this.positionsValid(tile.x.toInt(), tile.y.toInt(), other.x.toInt(), other.y.toInt())) {
      return (other.block() === tile.block() && tile.block() === this || tile.block() !is ItemNode && other.block() === this) && (other.team() === tile.team() || tile.block() !== this) && (!checkDouble || (other.build as ItemNodeBuild).link != tile.pos())
    } else {
      return false
    }
  }
  ...
  fun positionsValid(x1: Int, y1: Int, x2: Int, y2: Int): Boolean {
    if (x1 == x2) {
      return abs(y1 - y2) <= this.range
    } else if (y1 == y2) {
      return abs(x1 - x2) <= this.range
    } else {
      return false
    }
  }
  ...
  override fun changePlacementPath(points: Seq<Point2?>, rotation: Int) {
    Placement.calculateNodes(points, this, rotation, Boolf2 { point: Point2?, other: Point2? -> max(abs(point!!.x - other!!.x), abs(point.y - other.y)) <= this.range })
  }
  ...
  inner class ItemNodeBuild : SglBuilding(), Takeable {
    ...
    override fun drawSelect() {
      ...
      this.incoming.each(Intc { pos: Int -> this.drawInput(Vars.world.tile(pos)) })
      Draw.reset()
    }
    ...
    override fun buildConfiguration(table: Table) {
      ...
      table.table(Cons { t: Table? ->
        ...
        t.setOrigin(1)
        t.add().width(45.0f)
        (t.center().table(Tex.pane).get() as Table).add<DistTargetConfigTable?>(DistTargetConfigTable(0, this.config, if (this@ItemNode.siphon) arrayOf<GridChildType>(GridChildType.output, GridChildType.acceptor, GridChildType.input) else arrayOf<GridChildType>(GridChildType.output, GridChildType.acceptor), arrayOf<ContentType>(ContentType.item), true, Cons { c: TargetConfigure? ->
          c!!.offsetPos = 0
          this.configure(c.pack())
        }, Runnable { Vars.control.input.config.hideConfig() })).fill().center()
        t.top().button(Icon.info, Styles.grayi, 32.0f, Runnable {
    ...
    fun doSiphon() {
      if (this.config != null) {
        ...
        while (var1.hasNext()) {
          ...
          val other = this.getNext("siphonItem", Boolf { e: Building? -> e!!.interactable(this.team) && e.block.hasItems && e.items.has(item) && this.config!!.directValid(GridChildType.input, item, this.getDirectBit(e)) })
      ...
      if (this.config != null && !this.config!!.isClear) {
        ...
        while (var1.hasNext()) {
          ...
          if (this.items.get(i) > 0) {
            val next = this.getNext("items", Boolf { e: Building? -> e!!.interactable(this.team) && this.config!!.directValid(GridChildType.output, i, this.getDirectBit(e)) && e.acceptItem(this, i) })
            if (next != null) {
    ...
    protected fun checkAccept(source: Building, other: Tile?, content: UnlockableContent?): Boolean {
      if (this.tile != null && !this.linked(source) && this.config != null) {
        return if (this@ItemNode.linkValid(this.tile, other)) this.config!!.directValid(GridChildType.acceptor, content, this.getDirectBit(source)) else false
      } else {
        return true
      }
    }
