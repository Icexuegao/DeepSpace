...
open class SglBlock(name: String) : Block(name), ConsumerBlockComp, PostAtlasGenerator {
    ...
    companion object {
        ...
        private val fieldHandler = FieldHandler<PlacementFragment?>(PlacementFragment::class.java)
    }
    ...
    open inner class SglBuilding() : Building(), ConsumerBuildComp, NuclearEnergyBuildComp {
        ...
        fun drawActivation() {
            ...
            Fill.quad(
                x - w / 2 - 2, y + size * Vars.tilesize / 2f + h + 2,
                x - w / 2 - 2, y + size * Vars.tilesize / 2f - 2,
                x + w / 2 + 2, y + size * Vars.tilesize / 2f - 2,
                x + w / 2 + 2, y + size * Vars.tilesize / 2f + h + 2
            )
        ...
        override fun onOverpressure(potentialEnergy: Float) {
            activateRecover = 0f
            activation = Mathf.clamp(activation + min((potentialEnergy - maxEnergyPressure) / maxEnergyPressure * 0.01f, 0.008f) * Time.delta)
        }
        ...
        override fun acceptItem(source: Building, item: Item?): Boolean {
            return source.interactable(this.team) && hasItems
                    ...
                    || !(consumer.hasConsume() || consumer.hasOptional()) || consFilter.filter(this, ConsumeType.item, item, acceptAll(ConsumeType.item)))
        ...
        override fun acceptLiquid(source: Building, liquid: Liquid?): Boolean {
            return source.interactable(this.team) && hasLiquids
                    ...
                    || !(consumer.hasConsume() || consumer.hasOptional()) || consFilter.filter(this, ConsumeType.liquid, liquid, acceptAll(ConsumeType.liquid)))
