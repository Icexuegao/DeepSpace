...
open class Destructor(name: String) : NormalCrafter(name) {
  override fun init() {
    for (atomSchematic in SglContents.atomSchematics()) {
      ...
      consume!!.setConsTrigger { e: ConsumerBuildComp? ->
  ...
  override fun setBars() {
    ...
    addBar("progress") { e: NormalCrafterBuild? ->
      ...
      Bar({
        if (schematic != null) Core.bundle.format("bar.destructProgress", schematic.destructed(), schematic.researchConsume)
        else Core.bundle.get("bar.noSelect")
      }, { Pal.bar }, { schematic?.researchProgress() ?: 0f })
    }
  }
