...
class ReadItemsRequest(sender: DistElementBuildComp, private val destination: ItemsBuffer, private val reqItems: Seq<ItemStack>) :
  DistRequestBase(sender) {
  companion object {
  ...
  public override fun preHandleTask(): Boolean {
    ...
    for(stack in reqItems) {
      tempItems!![stack.item.id.toInt()] = min(
        stack.amount / total * destination.maxCapacity() - destination.get(stack.item), (stack.amount - source!!.get(stack.item)).toFloat()
      ).toInt()
    }
    ...
    itemFor@ for(id in tempItems!!.indices) {
      ...
      for(grid in target!!.grids) {
        for(entry in grid!!.get<Building>(
          GridChildType.container, Boolf2 { e: Building?, c: TargetConfigure? ->
            e!!.block.hasItems && e.items != null && e.items.get(item) > 0 && c!!.get(
              GridChildType.container, item
            )
          }, temp
        )) {
