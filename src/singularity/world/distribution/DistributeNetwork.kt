...
class DistributeNetwork : FinderContainerBase<DistElementBuildComp?>(), Iterable<DistElementBuildComp?> {
    ...
    var elements: TreeSeq<DistElementBuildComp> = TreeSeq<DistElementBuildComp>(Comparator { a: DistElementBuildComp, b: DistElementBuildComp -> if (b is DistNetworkCoreComp) 1 else b.priority() - a.priority() })
    ...
    var grids: TreeSeq<MatrixGrid?> = TreeSeq<MatrixGrid?>(Comparator { a: MatrixGrid?, b: MatrixGrid? -> b!!.priority - a!!.priority })
    ...
    companion object {
        ...
        val EMP_ARR: Array<DistElementBuildComp?> = arrayOfNulls<DistElementBuildComp>(0)
    }
}