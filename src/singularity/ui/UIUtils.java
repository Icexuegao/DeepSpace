...
public class UIUtils {
  ...
  public static void buildAmmo(Table table, BulletType bullet){
    ...
    if (bullet instanceof EmpBulletType emp){
      sep(table, Core.bundle.format("bullet.empDamage", emp.getEmpDamage(), emp.getEmpRange() > 0? "[lightgray]~ [accent]" + emp.getEmpRange() /tilesize + "[lightgray]" + StatUnit.blocks.localized() : ""));
    }
    ...
    if (bullet instanceof HeatBulletType heat){
      ...
      table.table(t -> {
        ...
        t.image(IStatus.INSTANCE.get熔毁() .uiIcon).size(25).scaling(Scaling.fit);
