...
class ProduceItems<T>(var items: Array<out ItemStack>) : BaseProduce<T>() where T : Building, T : ProducerBuildComp {
    ...
    public override fun type(): ProduceType<ProduceItems<*>>? {
    ...
    public override fun merge(other: BaseProduce<T>) {
        if (other is ProduceItems<*>) {
            ...
            items = TMP.values().toSeq().sort(Comparator { a: ItemStack?, b: ItemStack? -> a!!.item.id - b!!.item.id }).toArray<ItemStack?>(ItemStack::class.java)
            return
        }
    ...
    override fun display(stats: Stats) {
        stats.add(Stat.output, StatValue { table: Table? ->
            ...
            table.table(Cons { t: Table? ->
                ...
                if (!random) {
                    for (stack in items) {
                        val any: Table = if (showPerSecond) {
                            try {
                                val parent1 = parent!!
                                val cons = parent1.cons!!
                                val timePeriod = cons.craftTime
                                StatValues.displayItem(stack.item, stack.amount, timePeriod, true)
                            } catch (e: Exception) {
                            }
                            Table()
                        } else {
                    ...
                    t.table(Cons { item: Table? ->
                        for (stack in items) {
                            item!!.add<Table?>(StatValues.displayItem(stack.item, 0, true))
