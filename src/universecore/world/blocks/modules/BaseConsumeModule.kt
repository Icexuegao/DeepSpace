...
open class BaseConsumeModule(val entity: ConsumerBuildComp) : BlockModule() {
        ...
        if (current != null) {
            ...
            for (cons in current!!.all()) {
                ...
                if (preValid && entity.shouldConsume()) {
                    (cons as  BaseConsume<ConsumerBuildComp>).update(entity.getBuilding(ConsumerBuildComp::class.java))
                }
            }
        }
    ...
    fun updateOptional() {
        if (this.optional.isNotEmpty()) {
            var cons: BaseConsumers
            val onlyOne = entity.consumerBlock!!.oneOfOptionCons
            for (id in 0..<this.optional.size) {
                cons = this.optional.get(id)
                var optionalEff = 1f

                val valid1 = cons.valid

                for (b in valid1) {

                    val get = b!!.get(entity)

                    val f = if (get) 1f else 0f
                ...
                if (optionalEff > 0.0001f) {
                    ...
                    for (c in cons.all()) {
                        ...
                        if (c is ConsumePower<*>) powerCons += (c as ConsumePower<*>).requestedPower(entity.getBuild())
                    }
