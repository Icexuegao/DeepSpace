...
open class BaseConsumers(
    ...
    var consDelta: Floatf<ConsumerBuildComp> = Floatf {
        e: ConsumerBuildComp ->
    ...
    fun initFilter() {
        ...
        for (entry in cons) {
            ...
            if (cont != null) filter.get(entry.key, Prov { ObjectSet() })!!.addAll(cont)
        }
        for (entry in otherFilter) {
            filter.get(entry.key, Prov { ObjectSet() })!!.addAll(entry.value)
        }
    }
    ...
    fun addSelfAccess(type: ConsumeType<*>?, content: Content?) {
        selfAccess.get(type, Prov { ObjectSet() })!!.add(content)
    }
