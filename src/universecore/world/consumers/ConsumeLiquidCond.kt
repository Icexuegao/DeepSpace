...
open class ConsumeLiquidCond<T> : ConsumeLiquidBase<T>() where T : Building, T : ConsumerBuildComp {
    ...
    var usage: Float = 0f
    var filter: Boolf<Liquid?>? = null
    var usageMultiplier: Floatf<Liquid?> = Floatf { e: Liquid? -> 1f }
    var liquidEfficiency: Floatf<Liquid?> = Floatf { l: Liquid? -> 1f }
    ...
    val cons: Array<LiquidStack>
        get() {
            if (consLiquids == null) {
                ...
                consLiquids = seq.toArray<LiquidStack>(LiquidStack::class.java)
            }
    ...
    public override fun build(entity: T, table: Table) {
        val list = Vars.content.liquids().select { l: Liquid? -> !l!!.isHidden() && filter!!.get(l) }
    ...
    public override fun filter(): Seq<Content?>? {
        return Seq.with<LiquidStack?>(*this.cons).map<Content?> { s: LiquidStack? -> s!!.liquid }
    }
}