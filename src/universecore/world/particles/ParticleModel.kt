...
import universecore.world.particles.Particle.Companion.added
...
open class ParticleModel {
   ...
   * @param y 粒子创建时的y坐标
   * @param color 粒子的颜色
   * @param speedX 粒子初始运动速度的x分量
   * @param speedY 粒子初始运动速度的y分量
   * @param size 粒子的尺寸 */
  fun create(
    parent: Particle?, x: Float, y: Float, color: Color, speedX: Float, speedY: Float, size: Float, layer: Float = Layer.effect
  ): Particle {
  ...
  open fun reset(particle: Particle) {
    particle.apply {
      added = false
      parent = null
  ...
  open fun remove(particle: Particle) {
