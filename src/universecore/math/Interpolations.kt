 ...
 * @return [0f,1f]*/
val Progress.smooth: Progress get() = Interp.smooth(this.coerceIn(0f, 1f))
/**返回更平滑的插值。
 * @receiver 任意数字，最终被限制在 [0f,1f]
 * @return [0f,1f] */
val Progress.smoother: Progress get() = Interp.smoother(this.coerceIn(0f, 1f))
/**返回斜坡插值。
 * @receiver 任意数字，最终被限制在 [0f,1f] -> [1f,0f]
 * @return [0f,1f]*/
val Progress.slope: Progress get() = Interp.slope(this.coerceIn(0f, 1f))
/** 返回二次幂插值。
 * @receiver 任意数字，最终被限制在 [0f,1f]
 * @return [0f,1f] */
val Progress.pow2Intrp: Progress get() = Interp.pow2(this.coerceIn(0f, 1f))
/** 返回三次幂插值。
 * @receiver 任意数字，最终被限制在 [0f,1f]
 * @return [0f,1f] */
val Progress.pow3Intrp: Progress get() = Interp.pow3(this.coerceIn(0f, 1f))
/** 返回二次幂 Out 插值。
 * @receiver 任意数字，最终被限制在 [0f,1f]
 * @return [0f,1f] */
val Progress.pow2OutIntrp: Progress get() = Interp.pow2Out(this.coerceIn(0f, 1f))
/** 返回三次幂 Out 插值。
 * @receiver 任意数字，最终被限制在 [0f,1f]
 * @return [0f,1f] */
val Progress.pow3OutIntrp: Progress get() = Interp.pow3Out(this.coerceIn(0f, 1f))
/** 返回二次幂 In 插值。
 * @receiver 任意数字，最终被限制在 [0f,1f]
 * @return [0f,1f] */
val Progress.pow2InIntrp: Progress get() = Interp.pow2In(this.coerceIn(0f, 1f))
/** 返回三次幂 In 插值。
 * @receiver 任意数字，最终被限制在 [0f,1f]
 * @return [0f,1f] */
val Progress.pow3InIntrp: Progress get() = Interp.pow3In(this.coerceIn(0f, 1f))
/** @param growingTime 递增的数值
 * @param maxTime 最大边界
 * @return [0f,1f]，递增 */
fun progressTime(growingTime: Float, maxTime: Float): Progress = (growingTime / maxTime).coerceIn(0f, 1f)

fun dweep(time: Float) = progressTime(1f, 2f).smooth
/** 返回 1f - this */
