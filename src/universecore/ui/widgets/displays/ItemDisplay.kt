package universecore.ui.widgets.displays
  ...
  init {
    add(Stack().apply {
      ...
      if (amount != 0) {
        add(Table {t: Table ->
          t.left().bottom()
          t.add(if (amount >= 1000) UI.formatAmount(amount.toLong()) else amount.toString() + "").style(Styles.outlineLabel)
          t.pack()
        })
      }
    })
    withTooltip(this, item, true)
