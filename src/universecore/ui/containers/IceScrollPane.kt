package universecore.ui.containers
...
class IceScrollPane() : WidgetGroup() {
    ...
    private lateinit var style: ScrollPane.ScrollPaneStyle
    ...
    constructor(widget: Element, style: ScrollPane.ScrollPaneStyle) : this() {
        ...
        flickScrollListener = object : ElementGestureListener() {
            ...
            override fun handle(event: SceneEvent): Boolean {
                if (super.handle(event)) {
                    if ((event as InputEvent).type == InputEvent.InputEventType.touchDown) {
                        flingTimer = 0f
                    }
                    return true
                }
     ...
     * [.setStyle] is called.
     */
    fun getStyle(): ScrollPane.ScrollPaneStyle {
        return style
    }

    fun setStyle(style: ScrollPane.ScrollPaneStyle?) {
        requireNotNull(style) { "style cannot be null." }
        this.style = style
        invalidateHierarchy()
    }
     ...
     * based on [arc.scene.style.Drawable.getMinWidth] or [arc.scene.style.Drawable.getMinHeight]. Default is true.
     */
