...
abstract class LightningGenerator : Iterable<LightningVertex>, Iterator<LightningVertex> {
  ...
  var blockNow: Func2<LightningVertex?, LightningVertex?, Float?>? = null

  protected var curr: Lightning? = null
  ...
  fun createBranch(vertex: LightningVertex) {
    ...
    gen.blockNow = Func2 {l: LightningVertex?, v: LightningVertex? -> if (old != null) old.get(l, v) else if (blockNow != null) blockNow!!.get(l, v) else -1f}
  ...
  companion object {
    ...
    init {
      Pools.set(LightningVertex::class.java, object : Pool<LightningVertex?>(8192, 65536) {
