...
public class ToolUi {
    ...
    public static void toolUi() {
        Events.on(EventType.ClientLoadEvent.class, (e) -> {
            ...
            menu.button("\ue873", textBS, tool::editor).visible(() -> menuFirst && modeFirst).size(tableSize).checked(b -> Vars.state.rules.editor).tooltip("编辑模式");
            menu.button("\ue85b", textBS, tool::resources).visible(() -> menuFirst && modeFirst).size(tableSize).checked(b -> Vars.state.rules.infiniteResources).tooltip("沙盒模式");
