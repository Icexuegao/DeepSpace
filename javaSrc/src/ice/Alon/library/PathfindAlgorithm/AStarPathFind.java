...
import ice.Alon.library.sortAlgorithm.InsertionSorting;
...
public class AStarPathFind extends DrawUpdates.DrawUpdate {

    Tile target;
    Tile begin;
    int timer = 0;

    public boolean youhua = true;

    public AStarPathFind(Tile begin, Tile target) {
        this.begin = begin;
        this.target = target;
    ...
    public void draw() {
        ...
        Draw.alpha(0.4f);
        costMap.forEach((i, p)->{
            Fill.crect(p.x * Vars.tilesize - (float) Vars.tilesize / 2, p.y * Vars.tilesize - (float) Vars.tilesize / 2, Vars.tilesize, Vars.tilesize);
        });
       /* visited.each(p->{
