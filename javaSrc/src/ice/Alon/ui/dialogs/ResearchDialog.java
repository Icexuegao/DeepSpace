                            ...
                            if (selectable) {
                                ...
                                for (int i = 0; i < node.requirements.length; i++) {
                                    ...
                                    t.table(list -> {
                                        ...
                                        Label label = list.label(() -> " " + UI.formatAmount(Math.min(items.get(req.item), reqAmount)) + " / " + UI.formatAmount(reqAmount)).get();
        ...
        public void drawChildren() {
            ...
            for (TechTreeNode node : nodes) {
                ...
                for (TechTreeNode child : node.children) {
                    ...
                    Lines.stroke(Scl.scl(4f), lock ? Color.blue : Pal.lightishGray);
                    ...
                    if (Mathf.equal(Math.abs(node.y - child.y), Math.abs(node.x - child.x), 1f) && Mathf.dstm(node.x, node.y, child.x, child.y) <= node.width * 3) {
                        Lines.line(node.x + offsetX+60, node.y + offsetY+60, child.x + offsetX+60, child.y + offsetY+60);
                    } else {
                        Lines.line(node.x + offsetX+60, node.y + offsetY+60, child.x + offsetX+60, node.y + offsetY+60);
                        Lines.line(child.x + offsetX+60, node.y + offsetY+60, child.x + offsetX+60, child.y + offsetY+60);
                    }
                }
            }
