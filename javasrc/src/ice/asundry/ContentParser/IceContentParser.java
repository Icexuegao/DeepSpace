...
public class IceContentParser {
    ...
    ObjectSet<Class<?>> implicitNullable = ObjectSet.with(TextureRegion.class, TextureRegion[].class, TextureRegion[][].class, TextureRegion[][][].class);    ObjectMap<Class<?>, FieldParser> classParsers = new ObjectMap<>() {{
        ...
        put(StatusEffect.class, (type, data) -> {
            if (data.isString()) {
                ...
                throw new IllegalArgumentException("未知状态效果: '" + data.asString() + "'");
            }
        ...
        put(UnitCommand.class, (type, data) -> {
            if (data.isString()) {
                ...
                if (cmd != null) {
                    ...
                    throw new IllegalArgumentException("未知单位命令名称: " + data.asString());
                }
            } else {
        ...
        put(PartProgress.class, (type, data) -> {
            ...
            //无单数运算, check for multi-operation
            if (opval == null) {
        ...
        put(Mat3D.class, (type, data) -> {
            ...
            for (var val : data) {
                switch (val.name) {
                    ...
                    default -> throw new RuntimeException("未知矩阵变换: '" + val.name + "'");
                }
            }
    ...
    public Content parse(LoadedMod mod, String name, String json, Fi file, ContentType type) throws Exception {
        boolean icMod;
        icMod = mod == Vars.mods.getMod(Ice.class);
        if (contentTypes.isEmpty()) {
            init();
        }
        //删除额外的#字符使其成为有效json…  显然有些人在json中有未加引号的#字符//不是换行符???
        if (file.extension().equals("json")) {
            json = json.replace("#", "\\#");
        }
        currentMod = mod;
        JsonValue value = parser.fromJson(null, IceJval.read(json).toString(IceJval.Jformat.plain));
        if (!parsers.containsKey(type)) {
            throw new SerializationException("没有内容类型解析器:'" + type + "'");
        }
        boolean located = locate(type, name) != null;
        Content c = parsers.get(type).parse(icMod ? "" : mod.name + "-", name, value);
    ...
    private Prov<Unit> unitType(JsonValue value) {
        ...
        return switch (value.asString()) {
            ...
            default ->
                    throw new RuntimeException("无效的单元类型:'" + value + "'. 他必须是'flying/mech/legs/naval/payload/missile/tether/crawl'其中一种.");
        };
        ...
        private <T> T internalRead(Class<T> type, Class elementType, JsonValue jsonData, Class keyType) {
            if (type != null) {
                ...
                if (Content.class.isAssignableFrom(type)) {
                    ...
                    throw new IllegalArgumentException("\"" + jsonData.name + "\":" + "没有找到名字是'" + jsonData.asString() + "'的" + ctype + ".\n确保'" + jsonData.asString() + "'拼写正确,并且它确实存在!\n这也可能是因为它的文件无法解析!!!");
                }
            }
    ...
    private String getString(JsonValue value, String key) {
        if (value.has(key)) {
            ...
            throw new IllegalArgumentException("你少了一个 \"" + key + "\".必须在解析文件之前添加它.");
        }
    }
    ...
    private String getType(JsonValue value) {
        return getString(value, "type");
                ...
                if (locate(ContentType.block, name) != null) {
                    if (value.has("type")) {
                        Log.warn("警告:'" + currentMod.name + "-" + name + "'重新声明类型,这将被解析为一个新的方块,如果你想重写一个原版方块块,省略“类型”部分,因为原版方块的类型不能改变.");
                ...
                read(() -> {
                    if (value.has("consumes") && value.get("consumes").isObject()) {
                        for (JsonValue child : value.get("consumes")) {
                            switch (child.name) {
                                ...
                                case "items" ->
                                        block.consume(child.isArray() ? new ConsumeItems(parser.readValue(ItemStack[].class, child)) : parser.readValue(ConsumeItems.class, child));
                                ...
                                case "liquids" ->
                                        block.consume(child.isArray() ? new ConsumeLiquids(parser.readValue(LiquidStack[].class, child)) : parser.readValue(ConsumeLiquids.class, child));
                                ...
                                default ->
                                        throw new IllegalArgumentException("未知的消费类型:'" + child.name + "'在方块'" + block.name + "'中.");
                            }
                        }
                    ...
                    if (block.size > Vars.maxBlockSize) {
                        throw new IllegalArgumentException("方块是size不能大于" + Vars.maxBlockSize);
                    }

                    //如果有需求且未设置可见性，则默认设置方块为可见
                ...
                if (locate(ContentType.unit, name) == null) {
                    ...
                    if (unit.constructor == null || typeVal != null) {
                        if (typeVal != null && !typeVal.isString()) {
                            throw new RuntimeException("Unit的类型不正确:'" + name + "'类型必须是字符串.");
                        }
                ...
                read(() -> {
                    ...
                    if (value.has("requirements")) {
                        ...
                        if (req.block instanceof Reconstructor r) {
                            ...
                            throw new IllegalArgumentException("'block'缺少有效的'requirements'");
                        }
                ...
                if (locate(ContentType.item, name) != null) {
                    ...
                    readBundle(ContentType.item, name, value);
                } else {
                    readBundle(ContentType.item, name, value);
                    item = make(resolve(value.getString("type", null), Item.class), mod + name);
                    value.remove("type");
                }
                currentContent = item;
                read(() -> readFields(item, value));
            ...
            ContentType.sector, (TypeParser<SectorPreset>) (mod, name, value) -> {
                ...
                if (!value.has("sector") || !value.get("sector").isNumber())
                    throw new RuntimeException("SectorPresets必须有区块号.");
    ...
    private <T extends Content> T find(ContentType type, String name) {
        ...
        if (c == null) throw new IllegalArgumentException("没有找到名称是" + name + "的" + type);
        return (T) c;
    }
     ...
     *
     * @param //json要解析的json
     * @param //type内容的类型    * @param file该内容被解析的文件
     *                       * @返回已解析的内容
     */
    ...
    /**
     * 按名称从静态类获取字段, 如果未找到，则抛出描述性异常。
     */
    private Object field(Class<?> type, String name) {
        try {
            Object b = type.getField(name).get(null);
            if (b == null) throw new IllegalArgumentException(type.getSimpleName() + ": 不能找到:'" + name + "'");
            return b;
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    public Object fieldOpt(Class<?> type, JsonValue value) {
        try {
        ...
        for (JsonValue child = jsonMap.child; child != null; child = child.next) {
            ...
            if (metadata == null) {
                if (ignoreUnknownFields) {
                    Log.warn("@:[@]:忽略未知字段: @ (@)", currentMod.name, currentContent.minfo.sourceFile.name(), child.name, type.getSimpleName());
                    continue;
                } else {
                    SerializationException ex = new SerializationException("未找到字段: " + child.name + " (" + type.getName() + ")");
            ...
            try {
                ...
                throw new SerializationException("访问错误字段: " + field.getName() + " (" + type.getName() + ")", ex);
    ...
    <T> Class<T> resolve(String base, Class<T> def) {
        ...
        //如果在全局映射中找到，则返回映射类
        var out = IceClassMap.classes.get(!base.isEmpty() && Character.isLowerCase(base.charAt(0)) ? Strings.capitalize(base) : base);
        ...
        if (base.indexOf('.') != -1) {
            try {
                ...
                //尝试使用mod类加载器
                try {
                    return (Class<T>) Class.forName(base, true, Vars.mods.mainLoader());
        ...
        if (def != null) {
            Log.warn("[@] 没有找到名字是'" + base + "'的type , 默认为 '" + def.getSimpleName() + "'", currentContent == null ? currentMod.name : "");
            return def;
        }
        throw new IllegalArgumentException("找不到'" + base + "'类型,确保它拼写正确");
    }
