...
public class ADriPlanetGenerator extends PlanetGenerator {
    //交替的、较少直接的生成（wip）
    public static boolean alt = false;

    ADriBaseGenerator basegen = new ADriBaseGenerator();
    float scl = 5f;
    ...
    @Override
    public Color getColor(Vec3 position/**位置*/) {
    ...
    protected void generate() {
        ...
        class Room {
                        ...
                        for (int y = -rad; y <= rad; y++) {
                            ...
                            if (Structs.inBounds(wx, wy, width, height) && Mathf.within(x, y, rad)) {
                                ...
                                if (Mathf.within(x, y, rad - 1) && !other.floor().isLiquid) {
                                    //TODO 不尊重受污染的地板
                                    other.setFloor((Floor) (IceBlocks.redSandWater));
                                }
                            }
                        }
                    }
            ...
            void connectLiquid(Room to) /**连接液体*/
            {
                if (to == this) return;
        ...
        for (int i = 0; i < 360; i += angleStep) {
            ...
            for (int rx = -waterCheckRad; rx <= waterCheckRad; rx++) {
                for (int ry = -waterCheckRad; ry <= waterCheckRad; ry++) {
                    ...
                    if (tile == null || tile.floor() == IceBlocks.deepThickBlood || tile.floor() == IceBlocks.thickBlood || tile.floor().liquidDrop != null) {
                        waterTiles++;
                    }
                }
            }
        ...
        pass((x, y) -> {
            ...
            if (fspawn != null && value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)) {
                ...
                if (!(floor == Blocks.ice || floor == Blocks.iceSnow || floor == Blocks.snow || floor.asFloor().isLiquid)) {
                    floor = (deep ? IceBlocks.thickBlood : IceBlocks.redSandWater);
                }
            }
        });
        ...
        pass((x, y) -> {
            if (Math.abs(0.5f - noise(x, y, 6, 2, 111)) > 0.30) {
                if (floor == IceBlocks.iceRedIce && block == Blocks.air) {
                    block = ice.content.IceBlocks.bloodSporophore;
                }
            }
        });
        /**水晶簇随机 */
        pass((x, y) -> {
            if (Math.abs(0.5f - noise(x - 90, y, 4, 0.8, 65)) > 0.30) {
                if (floor == IceBlocks.iceRedIce) {
                    ...
                    if (i != 0) {
                        block = IceBlocks.bloodCrystalSpikes;
                    } else {
        ...
        if (naval) {
            ...
            pass((x, y) -> {
                if (floor.asFloor().isLiquid && !floor.asFloor().isDeep() && !floor.asFloor().shallow) {
                    ...
                    floor = floor == IceBlocks.thickBlood ? IceBlocks.deepThickBlood : IceBlocks.thickBlood;
                }
            });
        }
        ...
        pass((x, y) -> {
            ...
            if (floor == Blocks.hotrock) {
                ...
                if (noise > 0.67f && !roomseq.contains(e -> Mathf.within(x, y, e.x, e.y, 14))) {
                    if (noise > 0.72f) {
                        floor = noise > 0.78f ? Blocks.taintedWater : (IceBlocks.redSandWater);
                    } else {
        ...
        Vars.state.rules.spawns = Waves.generate(difficulty, new Rand(sector.id), Vars.state.rules.attackMode, Vars.state.rules.attackMode && Vars.spawner.countGroundSpawns() == 0, naval);
    }
    ...
    public void postGenerate(Tiles tiles) {
        if (sector.hasEnemyBase()) {
            ...
            if (Vars.spawner.countGroundSpawns() == 0) {
                Vars.state.rules.spawns = Waves.generate(sector.threat, new Rand(sector.id), Vars.state.rules.attackMode, true, false);
            }
        }
    }
}
