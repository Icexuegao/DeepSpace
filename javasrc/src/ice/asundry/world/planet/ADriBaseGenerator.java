...
public class ADriBaseGenerator {
    ...
    public static Block getDifficultyWall(int size, float difficulty) {
        Seq<Block> wallsSmall = Vars.content.blocks().select(b -> b instanceof Wall && b.isVanilla() && b.size == size && !b.insulated && b.buildVisibility == BuildVisibility.shown && !(b instanceof Door) && !(Structs.contains(b.requirements, i -> Vars.state.rules.hiddenBuildItems.contains(i.item))));
            ...
            for (Schematic.Stile tile : result.tiles) {
                ...
                if (tile.block instanceof Drill && (!insanity || !isTaken(tile.block, tile.x + cx, tile.y + cy))) {
                    ...
                    tile.block.iterateTaken(tile.x + cx, tile.y + cy, (ex, ey) -> {
                        ...
                        Tile rand = Vars.world.tiles.getc(ex + Mathf.range(1), ey + Mathf.range(1));
        ...
        if (part.required instanceof Item item) {
            for (Schematic.Stile tile : result.tiles) {
                if (tile.block instanceof Drill) {
                    ...
                    var build = Vars.world.build(tile.x + cx, tile.y + cy);
    ...
    static boolean overlaps(int x, int y) {
        ...
        return tile == null || !tile.block().alwaysReplace || Vars.world.getDarkness(x, y) > 0;
    }
    ...
    public void generate(Tiles tiles, Seq<Tile> cores, Tile spawn, Team team, Sector sector, float difficulty) {
        ...
        for (Tile tile : cores) {
            Astar.pathfind(tile, spawn, t -> t.team() == Vars.state.rules.waveTeam && !t.within(tile, coreDst) ? 100000 : t.floor().hasSurface() ? 1 : 10, t -> !t.block().isStatic()).each(t -> {
                if (!t.within(tile, coreDst)) {
                    ...
                    for (Point2 p : Geometry.d8) {
                        ...
                        if (other != null && other.team() == Vars.state.rules.waveTeam) {
    ...
    public void postGenerate() {
        ...
        for (Tile tile : tiles) {
            if (tile.isCenter() && tile.block() instanceof PowerNode && tile.team() == Vars.state.rules.waveTeam) {
